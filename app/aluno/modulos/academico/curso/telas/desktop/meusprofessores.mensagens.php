    ...
    <div class="span12">
        <div class="row-fluid">
            <div class="span12">
                ...
                <form action="<?php echo '/'.$url[0].'/'.$url[1].'/'.$url[2].'/'.$url[3].'/'.$url[4].'/mensagens';?>" id="form_iniciar_conversa" name="form_iniciar_conversa" method="post" enctype="multipart/form-data" onsubmit="return validateFields(this, regras);" target="_blank">
                    <input name="acao" id="acao" type="hidden" value="iniciar_chat" />
                    <input name="participantes[]" id="participantes" type="hidden" value="<?php echo 'PROFESSOR|'.$url[6]; ?>" />
