<?
class Grupos_Declaracoes extends Core
{

    function ListarTodas() {
        $this->sql = "SELECT ".$this->campos." FROM declaracoes_grupos where ativo='S'";
    ...
    function Retornar() {
        $this->sql = "SELECT ".$this->campos."
                            FROM
                             declaracoes_grupos where ativo='S' and idgrupo='".$this->id."'";
    ...
    function AssociarDeclaracoes($idgrupo, $arrayContrato) {
        foreach($arrayContrato as $ind => $idcontrato) {
            $this->sql = "select count(idgrupo_contrato) as total, idgrupo_contrato from declaracoes_grupos_declaracoes where idgrupo = '".intval($idgrupo)."' and idcontrato = '".intval($idcontrato)."'";
            $totalAss = $this->retornarLinha($this->sql);
            if($totalAss["total"] > 0) {
                $this->sql = "update declaracoes_grupos_declaracoes set ativo = 'S' where idgrupo_contrato = ".$totalAss["idgrupo_contrato"];
                $associar = $this->executaSql($this->sql);
                $this->monitora_qual = $totalAss["idgrupo_contrato"];
            } else {
                $this->sql = "insert into declaracoes_grupos_declaracoes set ativo = 'S', data_cad = now(), idgrupo = '".intval($idgrupo)."', idcontrato = '".intval($idcontrato)."'";
                $associar = $this->executaSql($this->sql);
    ...
    function DesassociarDeclaracoes() {
        ...
        if(!empty($erros)){
            ...
            $this->sql = "UPDATE declaracoes_grupos_declaracoes SET ativo = 'N' WHERE idgrupo_contrato = ".intval($this->post["remover"]);
    ...
    function ListarDeclaracoesAss() {
        $this->sql = "SELECT ".$this->campos." FROM
                            declaracoes_grupos_declaracoes cg
                            INNER JOIN declaracoes c ON (c.idcontrato  = cg.idcontrato)
                            INNER JOIN declaracoes_tipos ct ON (c.idtipo=ct.idtipo)
    ...
    function BuscarEmpreendimento($idgrupo) {
        ...
        $this->sql = "SELECT c.idcontrato AS 'key', CONCAT(c.nome, ' (', ct.nome, ')') AS value
                        ...
                        NOT EXISTS (
                                    SELECT g.idcontrato
                                    FROM declaracoes_grupos_declaracoes g
    ...
    function ListarDeclaracoesGrupo($idgrupo) {

        $this->sql = "SELECT c.idcontrato
                      FROM declaracoes_grupos_declaracoes cg
                         INNER JOIN declaracoes c ON (c.idcontrato  = cg.idcontrato)
