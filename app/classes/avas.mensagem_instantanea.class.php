...
class MensagemInstantanea extends Core
{
    ...
    */
    ...
    Variáveis:
        ...
        $this->idava: id do ava que será retornado as mensagem
    */
    function ListarMensagensInstantaneasProfessor() {
        $retorno = array();
        $this->sql = "SELECT
						".$this->campos."
					  FROM
						avas_mensagem_instantanea ami
						INNER JOIN avas_mensagem_instantanea_integrantes amii ON (amii.idmensagem_instantanea = ami.idmensagem_instantanea)
						INNER JOIN avas a ON ( ami.idava = a.idava )
                        INNER JOIN avas_mensagem_instantanea_integrantes amiia ON (amii.idmensagem_instantanea = ami.idmensagem_instantanea)
                        INNER JOIN pessoas p ON (amiia.idpessoa = p.idpessoa and  amiia.idmensagem_instantanea = ami.idmensagem_instantanea AND
                            amiia.idpessoa IS NOT NULL )
					  WHERE
						amii.idprofessor = '".$this->idprofessor."' AND
						a.ativo = 'S' AND a.ativo_painel = 'S' AND
						ami.ativo = 'S' AND
						amii.ativo = 'S' ";
                    ...
                    if ($campo[0] == 1) {
                        ...
                        $this->sql .= " and ((SELECT 
								    		...
								    		WHERE 
								    			...
								    			amiia.idpessoa IS NOT NULL 
								    		ORDER BY amiia.data_cad ASC LIMIT 1) LIKE '%".$valor."%' ) ";

                    }
                }
            }
        }
        $this->groupby = 'ami.idmensagem_instantanea';
        $this->mantem_groupby = true;
        return $this->retornarLinhasTD();
    }
    ...
    public function getAvasProfessor($idProfessor){
        #AVAS
        $sqlProfAva = "SELECT idava FROM professores_avas 
		WHERE idprofessor = '".$idProfessor."' AND ativo = 'S' LIMIT 100 ";
        $queryProfAva = $this->executaSql($sqlProfAva);
        while ($linhaProfAva = mysql_fetch_assoc($queryProfAva)) {
            $retornoA[] = $linhaProfAva['idava'];
        }
        return $retornoA;
    }

    public function getCursosProfessor($idProfessor){
        #CURSOS
        $sqlProfCurso = "SELECT idcurso FROM professores_cursos 
		WHERE idprofessor = '".$idProfessor."' AND ativo = 'S' LIMIT 100 ";
        $queryProfCurso = $this->executaSql($sqlProfCurso);
        while ($linhaProfCurso = mysql_fetch_assoc($queryProfCurso)) {
            $retornoC[] = $linhaProfCurso['idcurso'];
        }
        return $retornoC;
    }
    ...
    public function getDisciplinasProfessor($idProfessor){
        #DISCIPLINAS
        $sqlProfDisc = "SELECT iddisciplina FROM professores_disciplinas 
		WHERE idprofessor = '".$idProfessor."' AND ativo = 'S' LIMIT 100 ";
        $queryProfDisc = $this->executaSql($sqlProfDisc);
        while ($linhaProfDisc = mysql_fetch_assoc($queryProfDisc)) {
            $retornoD[] = $linhaProfDisc['iddisciplina'];
        }
        return $retornoD;
    }

    public function getOfertasProfessor($idProfessor){
        #DISCIPLINAS
        $sqlProfOferta = "SELECT idoferta FROM professores_ofertas 
		WHERE idprofessor = '".$idProfessor."' AND ativo = 'S' LIMIT 100 ";
        $queryProfOferta = $this->executaSql($sqlProfOferta);
        while ($linhaProfOferta = mysql_fetch_assoc($queryProfOferta)) {
            $retornoO[] = $linhaProfOferta['idoferta'];
        }
        return $retornoO;
    }
    ...
    */
    function adicionarUsuarioMensagem()
    {
        if (verificaPermissaoAcesso(true)) {
            $this->retorno = array();
    ...
    */
    ...
    function RetornarMensagemInstantanea() {
        $this->sql = "SELECT * FROM avas_mensagem_instantanea WHERE idmensagem_instantanea = '".$this->idmensagem_instantanea."'";
        return $this->retornarLinha($this->sql);
    }

    function Sinalizador($idmensagem_instantanea, $sim_nao) {
        $this->sql = 'update avas_mensagem_instantanea set sinalizador_professor = "'.$sim_nao.'" where idmensagem_instantanea = '.$idmensagem_instantanea;
