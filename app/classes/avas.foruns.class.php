...
class Foruns extends Ava {
  ...
  function ListarTodasForum() {  
    ...
    foreach($foruns as $ind => $forum) {
      ...
      $foruns[$ind]["topicos"] = $this->ListarTodasTopico($forum["idforum"], $permissaoTopicoOculto);   
      
      $foruns[$ind]["total_topicos"] = $this->RetornarTotalTopicos($forum["idforum"]);
      $foruns[$ind]["total_respostas"] = $this->RetornarTotalRespostas($forum["idforum"]);
    }

    // $foruns['participantes'] = $this->ListarParticipantesForum();
  ...
  function InsereAssinantesMensagens($idtopico, $enviarEmail = true){
   ...
   function EnviaEmailForum($idtopico){   
        ...
        $nomeDe = utf8_decode($GLOBALS['config']['tituloEmpresa']);                 
        $assunto = utf8_decode($lnEmail['nome']);                       
       ...
       // INICIO RETORNA TODAS AS PESSOAS QUE SÃO ASSINANTES DO TOPICO E ENVIA O EMAIL // LEFT JOIN professores p ON (aftam.idprofessor = p.idprofessor AND pe.ativo = 'S') , IF(ISNULL(p.nome),pe.nome,p.nome) as nome, IF(ISNULL(p.email),pe.email,p.email) as email
       $this->sql = "SELECT pe.nome,pe.email,aftam.code
