...
class BoletosGerados extends Core {
  ...
  function ListarTodas() {      
    ...
    $this->sql = "SELECT ".$this->campos." 
                ...
                WHERE c.forma_pagamento = 1 AND c.ativo = 'S' ";
    ...
    if($evento['eventos']){
        $this->sql .= " and c.idevento in (".$evento['eventos'].") ";
    }
  ...
  function TipoMensalidade() {
    $this->sql = "select idevento from eventos_financeiros where ativo = 'S' and mensalidade  = 'S' ";          
    return $this->retornarLinha($this->sql);
  }
  
  function retornarTiposEventos(){
   $this->sql = "SELECT GROUP_CONCAT(idevento SEPARATOR ', ') eventos 
                 FROM eventos_financeiros 
                 WHERE (mensalidade = 'S' or taxa_reativacao = 'S') AND ativo = 'S'";
   return $this->retornarLinha($this->sql);
  }
