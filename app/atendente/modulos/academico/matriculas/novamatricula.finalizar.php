...
$pessoaObj->Set("idvendedor",$usu_vendedor["idvendedor"]);
$pessoaObj->Set("monitora_onde",$config["monitoramento_pessoa"]["onde"]);

//unset($config['formulario_pessoas'][0]['campos'][0]); 
//unset($config['formulario_pessoas'][0]['campos'][1]);     
...
if($_SESSION["matricula"]["pessoa"]["idpessoa"]) {
  $pessoa = $pessoaObj->Modificar();
    ...
    $config["formulario_pessoas"][1]["campos"][] = array(
    "id" => "form_senha",
    "nome" => "senha",
    "tipo" => "input", 
    "valor" => "senha",
