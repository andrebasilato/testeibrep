...
<table width="100%" height="100%" border="0" cellspacing="0" cellpadding="10">
  <tr>
    <td align="left"> 
      ...
      <form action="/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/<?= $url[3]; ?>/administrar" method="post" class="form-horizontal" target="_parent" onSubmit="return confirmaAlteracao()">
        <input name="acao" type="hidden" value="alterar_situacao">
        <input name="situacao_para" type="hidden" value="<?php echo $situacaoInativa["idsituacao"]; ?>">
          ...
          <tr>  
            <td align="right" valign="middle">
              <select name="idmotivo" id="idmotivo">
                <option value=""></option>
                <?php if(is_array($motivosInativar)) {
                  foreach($motivosInativar as $motivoInativar){ ?>
                    <option value="<?= $motivoInativar['idmotivo']; ?>"><?=$motivoInativar['nome'];?></option>
                  <? }	 
                }
                ?>
              </select>
