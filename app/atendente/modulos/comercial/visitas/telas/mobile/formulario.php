        ...
        <div class="box-conteudo">
      		...
      		<form method="post" onsubmit="return validateFields(this, regras)" enctype="multipart/form-data" class="form-horizontal">
				...
				?>
                    ...
                    <div class="controls">
                         <div id="map-canvas" style="display:none; margin-top: 25px;height: 350px"></div>
                    </div>
                </div>
	...
	<?
			...
			if($campo["json"]){ ?>
					$('#<?=$campo["json_idpai"];?>').change(function(){
						...
						if ('<?=$campo["json_idpai"];?>' == 'idregiao')
							$('#idempreendimento').html('<option value="">– <?=$idioma['form_selecione_empreendimento']; ?> –</option>');					
					});
...
<script type="text/javascript">
    ...
    createMap();	                
		...
		if(<?=intval($linha["idpessoa"])?> > 0)
				$("#form_idpessoa").trigger("addItem",[{"title": "<?=$linha["nome"]?>", "value": "<?=$linha["idpessoa"]?>"}]);
	});
</script>
</div>
</body>
