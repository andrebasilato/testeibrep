                        ...
                        <div class="tab-pane active" id="tab_editar">
                            ...
                            <form class="well form-inline" method="post">
                                <?php if ($perfil["permissoes"][$url[2] . "|7"]) { ?>
                                    <label class="checkbox">
                                        <input type="checkbox" value="S" id="gestor_sindicato"
                                               name="gestor_sindicato"
                                               <?php if ($linha["gestor_sindicato"] == "S") { ?>checked="checked"<?php } ?>> <?= $idioma["form_gestor"]; ?>
                                    </label>
                                    <p class="help-block"><?= $idioma["form_gestor_ajuda"]; ?></p>
                                    <br/>
                                    <input type="hidden" id="acao" name="acao" value="gestor_sindicato">
                                    <input type="submit" class="btn" value="<?= $idioma["btn_salvar"]; ?>"/>
                                <?php } else { ?>
                                    <label class="checkbox">
                                        <input type="checkbox" value="S" id="gestor_sindicato"
                                               name="gestor_sindicato"
                                               <?php if ($linha["gestor_sindicato"] == "S") { ?>checked="checked"<?php } ?>
                            ...
                            <table class="table table-striped tabelaSemTamanho">
                                ...
                                <?php if (count($sindicatos) > 0) { ?>
                                    <?php foreach ($sindicatos as $ind => $sindicato) { ?>
                                        <tr>
                                            <td><?php echo $sindicato["idsindicato"]; ?>
                                                - <?php echo $sindicato["nome_abreviado"]; ?></td>
                                            <td>
                                                <?php if ($perfil["permissoes"][$url[2] . "|8"]) { ?>
                                                    <a href="javascript:void(0);" class="btn btn-mini"
                                                       ...
                                                       onclick="remover(<?php echo $sindicato["idusuario_sindicato"]; ?>)"><i
