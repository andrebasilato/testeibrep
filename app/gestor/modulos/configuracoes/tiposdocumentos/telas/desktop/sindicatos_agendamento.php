                    ...
                    <div class="tab-content">
                        <div class="tab-pane active" id="tab_editar">
                            ...
                            <form class="well form-inline" method="post">
                                <?php if ($perfil["permissoes"][$url[2] . "|11"]) { ?>
                                    <label class="checkbox">
                                        <input type="checkbox" value="S" id="todas_sindicatos_obrigatorio_agendamento"
                                               name="todas_sindicatos_obrigatorio_agendamento"
                                               <?php if ($linha["todas_sindicatos_obrigatorio_agendamento"] == "S") { ?>checked="checked"<?php } ?>> <?= $idioma["form_todas"]; ?>
                                    </label>
                                    <p class="help-block"><?= $idioma["form_todas_ajuda"]; ?></p>
                                    <br/>
                                    <input type="hidden" id="acao" name="acao" value="todas_sindicatos_agendamento_obrigatorio">
                                    <input type="submit" class="btn" value="<?= $idioma["btn_salvar"]; ?>"/>
                                <?php } else { ?>
                                    <label class="checkbox">
                                        <input type="checkbox" value="S" id="todas_sindicatos_obrigatorio_agendamento"
                                               name="todas_sindicatos_obrigatorio_agendamento"
                                               <?php if ($linha["todas_sindicatos_obrigatorio_agendamento"] == "S") { ?>checked="checked"<?php } ?>
                                               disabled="disabled"> <?= $idioma["form_todas"]; ?>
                                    </label>
                                    <p class="help-block"><?= $idioma["form_todas_ajuda"]; ?></p>
                                    <br/>
                            ...
                            <form class="well" method="post">
                                <p><?= $idioma["form_instituiao"]; ?></p>
                                <?php if ($perfil["permissoes"][$url[2] . "|11"]) { ?>
                                    <select id="sindicatos" name="sindicatos"></select>
                                    <br/>
                                    <br/>
                                    <input type="hidden" id="acao" name="acao" value="associar_sindicato_agendamento">
                            ...
                            <form method="post" id="remover_sindicato" name="remover_sindicato">
                                <input type="hidden" id="acao" name="acao" value="remover_sindicato_agendamento">
                            ...
                            <table class="table table-striped tabelaSemTamanho">
                                ...
                                <tbody>
                                <?php if (count($sindicatosArray) > 0) { ?>
                                    <?php foreach ($sindicatosArray as $ind => $sindicato) { ?>
                                        <tr>
                                            <td><?php echo $sindicato["idsindicato"]; ?></td>
                                            ...
                                            <td>
                                                <?php if ($perfil["permissoes"][$url[2] . "|12"]) { ?>
                                                    <a href="javascript:void(0);" class="btn btn-mini"
                                                       ...
                                                       onclick="remover(<?php echo $sindicato["idtipo_sindicato"]; ?>)"><i
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            $("#sindicatos").fcbkcomplete({
                json_url: "/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/<?= $url[3]; ?>/json/associar_sindicatos_agendamento",
                addontab: true,
                height: 10,
                maxshownitems: 10,
                cache: true,
