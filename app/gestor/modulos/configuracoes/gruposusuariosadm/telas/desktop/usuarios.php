                    ...
                    <div class="tab-content">
                        <div class="tab-pane active" id="tab_editar">
                            ...
                            <form class="well" method="post">
                                <p><?= $idioma["form_usuario"]; ?></p>
                                <?php if ($perfil["permissoes"][$url[2] . "|5"]) { ?>
                                    <select id="usuarios" name="usuarios"></select>
                                    <br/>
                                    <br/>
                                    <input type="hidden" id="acao" name="acao" value="associar_usuario">
                                    <input type="submit" class="btn" value="<?= $idioma["btn_adicionar"]; ?>"/>
                                <?php } else { ?>
                                    <select id="usuarios" name="usuarios" disabled="disabled"></select>
                                    <br/>
                                    <br/>
                            ...
                            <form method="post" id="remover_usuario" name="remover_usuario">
                                <input type="hidden" id="acao" name="acao" value="remover_usuario">
                            ...
                            <table class="table table-striped">
                                ...
                                <tbody>
                                <?php if (count($usuariosArray) > 0) { ?>
                                    <?php foreach ($usuariosArray as $ind => $usuario) { ?>
                                        <tr>
                                            ...
                                            <td>
                                                <?php if ($perfil["permissoes"][$url[2] . "|6"]) { ?>
                                                    <a href="javascript:void(0);" class="btn btn-mini"
                                                       ...
                                                       onclick="remover(<?php echo $usuario["idgrupo_usuario"]; ?>)"><i
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            $("#usuarios").fcbkcomplete({
                json_url: "/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/<?= $url[3]; ?>/associar_usuarios",
                addontab: true,
                height: 10,
                maxshownitems: 10,
                cache: true,
        ...
        function remover(id) {
            ...
            if (confirma) {
                ...
                document.getElementById("remover_usuario").submit();
            }
        }
    </script>
</div>
