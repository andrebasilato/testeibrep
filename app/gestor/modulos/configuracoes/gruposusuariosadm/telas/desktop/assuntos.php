...
<div class="container-fluid">
                    ...
                    <div class="tab-content">
                        <div class="tab-pane active" id="tab_editar">
                            ...
                            <form class="well" method="post">
                                <p><?= $idioma["form_assunto"]; ?></p>
                                <?php if ($perfil["permissoes"][$url[2] . "|8"]) { ?>
                                    <select id="assuntos" name="assuntos"></select>
                                    <br/>
                                    <br/>
                                    <input type="hidden" id="acao" name="acao" value="associar_assunto">
                                    <input type="submit" class="btn" value="<?= $idioma["btn_adicionar"]; ?>"/>
                                <?php } else { ?>
                                    <select id="assuntos" name="assuntos" disabled="disabled"></select>
                                    <br/>
                                    <br/>
                            ...
                            <form method="post" id="remover_assunto" name="remover_assunto">
                                <input type="hidden" id="acao" name="acao" value="remover_assunto">
                            ...
                            <table class="table table-striped">
                                ...
                                <tbody>
                                <?php if (count($assuntosArray) > 0) { ?>
                                    <?php foreach ($assuntosArray as $ind => $assunto) { ?>
                                        <tr>
                                            <td><?php echo $assunto["nome"]; ?></td>
                                            <td>
                                                <?php if ($perfil["permissoes"][$url[2] . "|9"]) { ?>
                                                    <a href="javascript:void(0);" class="btn btn-mini"
                                                       ...
                                                       onclick="remover(<?php echo $assunto["idassunto_grupo"]; ?>)"><i
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            $("#assuntos").fcbkcomplete({
                json_url: "/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/<?= $url[3]; ?>/associar_assuntos",
                addontab: true,
                height: 10,
                maxshownitems: 10,
                cache: true,
        ...
        function remover(id) {
            ...
            if (confirma) {
                ...
                document.getElementById("remover_assunto").submit();
            }
        }
    </script>
</div>
