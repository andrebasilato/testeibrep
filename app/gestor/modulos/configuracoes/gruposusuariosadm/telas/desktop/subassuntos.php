...
<div class="container-fluid">
                    ...
                    <div class="tab-content">
                        <div class="tab-pane active" id="tab_editar">
                            ...
                            <form class="well" method="post">
                                <p><?= $idioma["form_subassunto"]; ?></p>
                                <?php if ($perfil["permissoes"][$url[2] . "|11"]) { ?>
                                    <select id="subassuntos" name="subassuntos"></select>
                                    <br/>
                                    <br/>
                                    <input type="hidden" id="acao" name="acao" value="associar_subassunto">
                                    <input type="submit" class="btn" value="<?= $idioma["btn_adicionar"]; ?>"/>
                                <?php } else { ?>
                                    <select id="subassuntos" name="subassuntos" disabled="disabled"></select>
                                    <br/>
                                    <br/>
                            ...
                            <form method="post" id="remover_subassunto" name="remover_subassunto">
                                <input type="hidden" id="acao" name="acao" value="remover_subassunto">
                                <input type="hidden" id="remover" name="remover" value="">
                            </form>
                            <table class="table table-striped">
                                <thead>
                                <tr>
                                    <th><?= $idioma["listagem_subassunto"]; ?></th>
                                    <th><?= $idioma["listagem_assunto"]; ?></th>
                                    <th width="60"><?= $idioma["listagem_opcoes"]; ?></th>
                                </tr>
                                </thead>
                                <tbody>
                                <?php if (count($subassuntosArray) > 0) { ?>
                                    <?php foreach ($subassuntosArray as $ind => $subassunto) { ?>
                                        <tr>
                                            <td><?php echo $subassunto["nome"]; ?></td>
                                            <td><?php echo $subassunto["assunto"]; ?></td>
                                            <td>
                                                <?php if ($perfil["permissoes"][$url[2] . "|12"]) { ?>
                                                    <a href="javascript:void(0);" class="btn btn-mini"
                                                       ...
                                                       onclick="remover(<?php echo $subassunto["idsubassunto_grupo"]; ?>)"><i
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            $("#subassuntos").fcbkcomplete({
                json_url: "/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/<?= $url[3]; ?>/associar_subassuntos",
                addontab: true,
                height: 10,
                maxshownitems: 10,
                cache: true,
        ...
        function remover(id) {
            ...
            if (confirma) {
                ...
                document.getElementById("remover_subassunto").submit();
            }
        }
    </script>
</div>
