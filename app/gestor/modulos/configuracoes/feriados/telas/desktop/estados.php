...
<div class="container-fluid">
                    ...
                    <div class="tab-content">
                        <div class="tab-pane active" id="tab_editar">
                            ...
                            <form class="well" method="post">
                                ...
                                <p><?= $idioma["form_estados"]; ?></p>
                                <?php if ($perfil["permissoes"][$url[2] . "|8"]) { ?>
                                    <select id="estados" name="estados"></select>
                                    <br/>
                                    <br/>
                                    <input type="hidden" id="acao" name="acao" value="associar_estado">
                                    <input type="submit" class="btn" value="<?= $idioma["btn_adicionar"]; ?>"/>
                                <?php } else { ?>
                                    <select id="estados" name="estados" disabled="disabled"></select>
                                    <br/>
                                    <br/>
                            ...
                            <form method="post" id="remover_associacao_estado" name="remover_associacao_estado">
                                <input type="hidden" id="acao" name="acao" value="remover_associacao_estado">
                            ...
                            <table class="table table-striped tabelaSemTamanho">
                                ...
                                <?php if (count($associacoesArray) > 0) { ?>
                                    <?php foreach ($associacoesArray as $ind => $associacao) { ?>
                                        <tr>
                                            <td><?php echo $associacao["idferiado_estado"]; ?></td>
                                            ...
                                            <td>
                                                <?php if ($perfil["permissoes"][$url[2] . "|9"]) { ?>
                                                    <a href="javascript:void(0);" class="btn btn-mini"
                                                       ...
                                                       onclick="remover(<?php echo $associacao["idferiado_estado"]; ?>)"><i
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            $("#estados").fcbkcomplete({
                json_url: "/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/<?= $url[3]; ?>/json/associar_estados",
                addontab: true,
                height: 10,
                maxshownitems: 10,
                cache: true,
        ...
        function remover(id) {
            ...
            if (confirma) {
                ...
                document.getElementById("remover_associacao_estado").submit();
            }
        }
    </script>
</div>
