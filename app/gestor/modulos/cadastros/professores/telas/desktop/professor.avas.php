...
<div class="container-fluid">
              		...
              		<div class="tab-content">
                		<div class="tab-pane active" id="tab_editar">
    						...
    						<form class="well" method="post">
                                ...
                                <p><?= $idioma["form_avas"]; ?></p>
								<?php if($perfil["permissoes"][$url[2]."|6"]) { ?>    
                                <select id="avas" name="avas"></select>
                                <br />
                                <br />
    							<input type="hidden" id="acao" name="acao" value="associar_ava">
                                <input type="submit" class="btn" value="<?= $idioma["btn_adicionar"]; ?>" />
                                <?php } else { ?>
                                <select id="avas" name="avas" disabled="disabled"></select>
                                <br />
                                <br />
                            ...
                            <br />
     						<form method="post" id="remover_associacao_ava" name="remover_associacao_ava">
    							<input type="hidden" id="acao" name="acao" value="remover_associacao_ava">
        						...
        						<tbody>
                                    <?php if(count($associacoesArray) > 0) { ?>
										<?php foreach($associacoesArray as $ind => $associacao) { ?>
                                            <tr>
                                                <td><?php echo $associacao["idprofessor_ava"]; ?></td>
                                                <td><?php echo $associacao["nome"]; ?></td>
                                                <td>
                                                  <?php if($perfil["permissoes"][$url[2]."|7"]) { ?>
                                                  <a href="javascript:void(0);"  class="btn btn-mini" data-original-title="<?= $idioma["btn_remover"]; ?>" data-placement="left" rel="tooltip" onclick="remover(<?php echo $associacao["idprofessor_ava"]; ?>)"><i class="icon-remove"></i></a>
                                                  <?php } else { ?>
    ...
    <script type="text/javascript">
		$(document).ready(function(){                
			$("#avas").fcbkcomplete({
				json_url: "/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/<?= $url[3]; ?>/json/associar_avas",
				addontab: true,
				height: 10,
				maxshownitems: 10,
				cache: true,
		...
		function remover(id) {
			...
			if(confirma) {
				...
				document.getElementById("remover_associacao_ava").submit();
			} 
		}
	</script>
</div>
