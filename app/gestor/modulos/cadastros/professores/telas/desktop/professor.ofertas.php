...
<div class="container-fluid">
              		...
              		<div class="tab-content">
                		<div class="tab-pane active" id="tab_editar">
    						...
    						<form class="well" method="post">
                                ...
                                <p><?= $idioma["form_ofertas"]; ?></p>
								<?php if($perfil["permissoes"][$url[2]."|6"]) { ?>    
                                <select id="ofertas" name="ofertas"></select>
                                <br />
                                <br />
    							<input type="hidden" id="acao" name="acao" value="associar_oferta">
                                <input type="submit" class="btn" value="<?= $idioma["btn_adicionar"]; ?>" />
                                <?php } else { ?>
                                <select id="ofertas" name="ofertas" disabled="disabled"></select>
                                <br />
                                <br />
                            ...
                            <br />
     						<form method="post" id="remover_associacao_oferta" name="remover_associacao_oferta">
    							<input type="hidden" id="acao" name="acao" value="remover_associacao_oferta">
        						...
        						<tbody>
                                    <?php if(count($associacoesArray) > 0) { ?>
										<?php foreach($associacoesArray as $ind => $associacao) { ?>
                                            <tr>
                                                <td><?php echo $associacao["idprofessor_oferta"]; ?></td>
                                                <td><?php echo $associacao["nome"]; ?></td>
                                                <td>
                                                  <?php if($perfil["permissoes"][$url[2]."|7"]) { ?>
                                                  <a href="javascript:void(0);"  class="btn btn-mini" data-original-title="<?= $idioma["btn_remover"]; ?>" data-placement="left" rel="tooltip" onclick="remover(<?php echo $associacao["idprofessor_oferta"]; ?>)"><i class="icon-remove"></i></a>
                                                  <?php } else { ?>
    ...
    <script type="text/javascript">
		$(document).ready(function(){                
			$("#ofertas").fcbkcomplete({
				json_url: "/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/<?= $url[3]; ?>/json/associar_ofertas",
				addontab: true,
				height: 10,
				maxshownitems: 10,
				cache: true,
		...
		function remover(id) {
			...
			if(confirma) {
				...
				document.getElementById("remover_associacao_oferta").submit();
			} 
		}
	</script>
</div>
