...
<div class="container-fluid">
              		...
              		<div class="tab-content">
                		<div class="tab-pane active" id="tab_editar">
    						...
    						<form class="well" method="post">
                                ...
                                <p><?= $idioma["form_disciplinas"]; ?></p>
								<?php if($perfil["permissoes"][$url[2]."|13"]) { ?>    
                                <select id="disciplinas" name="disciplinas"></select>
                                <br />
                                <br />
    							<input type="hidden" id="acao" name="acao" value="associar_disciplina">
                                <input type="submit" class="btn" value="<?= $idioma["btn_adicionar"]; ?>" />
                                <?php } else { ?>
                                <select id="disciplinas" name="disciplinas" disabled="disabled"></select>
                                <br />
                                <br />
                            ...
                            <br />
     						<form method="post" id="remover_associacao_disciplina" name="remover_associacao_disciplina">
    							<input type="hidden" id="acao" name="acao" value="remover_associacao_disciplina">
        						...
        						<tbody>
                                    <?php if(count($associacoesArray) > 0) { ?>
										<?php foreach($associacoesArray as $ind => $associacao) { ?>
                                            <tr>
                                                <td><?php echo $associacao["idprofessor_disciplina"]; ?></td>
                                                <td><?php echo $associacao["nome"]; ?></td>
                                                <td>
                                                  <?php if($perfil["permissoes"][$url[2]."|14"]) { ?>
                                                  <a href="javascript:void(0);"  class="btn btn-mini" data-original-title="<?= $idioma["btn_remover"]; ?>" data-placement="left" rel="tooltip" onclick="remover(<?php echo $associacao["idprofessor_disciplina"]; ?>)"><i class="icon-remove"></i></a>
                                                  <?php } else { ?>
    ...
    <script type="text/javascript">
		$(document).ready(function(){                
			$("#disciplinas").fcbkcomplete({
				json_url: "/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/<?= $url[3]; ?>/json/associar_disciplinas",
				addontab: true,
				height: 10,
				maxshownitems: 10,
				cache: true,
		...
		function remover(id) {
			...
			if(confirma) {
				...
				document.getElementById("remover_associacao_disciplina").submit();
			} 
		}
	</script>
</div>
