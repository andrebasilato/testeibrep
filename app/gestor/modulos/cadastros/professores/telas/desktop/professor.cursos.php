			 		...
			 		<?php incluirTela("inc_menu_edicao",$config,$usuario); ?>
              		<div class="tab-content">
                		<div class="tab-pane active" id="tab_editar">
                            ...
                            <br />
     						<form method="post" id="remover_associacao_curso" name="remover_associacao_curso">
    							<input type="hidden" id="acao" name="acao" value="remover_associacao_curso">
        						...
        						<tbody>
                                    <?php if(count($associacoesArray) > 0) { ?>
										<?php foreach($associacoesArray as $ind => $associacao) { ?>
                                            <tr>
                                                <td><?php echo $associacao["idprofessor_curso"]; ?></td>
                                                <td><?php echo $associacao["nome"]; ?></td>
                                                <td>
                                                  <?php if($perfil["permissoes"][$url[2]."|7"]) { ?>
                                                  <a href="javascript:void(0);"  class="btn btn-mini" data-original-title="<?= $idioma["btn_remover"]; ?>" data-placement="left" rel="tooltip" onclick="remover(<?php echo $associacao["idprofessor_curso"]; ?>)"><i class="icon-remove"></i></a>
                                                  <?php } else { ?>
    ...
    <script type="text/javascript">
		...
		function remover(id) {
			...
			if(confirma) {
				...
				document.getElementById("remover_associacao_curso").submit();
			} 
		}
	</script>
</div>
