  ...
  <div class="row-fluid">
          ...
          <div class="tabbable tabs-left">
			<?php if($url[3] != "cadastrar") { ?>
                <table border="0" cellspacing="0" cellpadding="8">
                    <tr>
                        <td>
                            <img src="/api/get/imagens/pessoas_avatar/x/120/<?php echo $linha["avatar_servidor"]; ?>" />
                        </td>
                        <td>
                ...
                <form method="post" onsubmit="return validateFields(this, regras)" enctype="multipart/form-data" class="form-horizontal">
				  ...
				  <? if($url[4] == "editar") {
					...
					foreach($config["banco"]["campos_unicos"] as $campoid => $campo) {
					  if($campo["campo_form"] != 'documento') {
					  ?>
						<input name="<?= $campo["campo_form"]; ?>_antigo" id="<?= $campo["campo_form"]; ?>_antigo" type="hidden" value="<?= $linha[$campo["campo_banco"]]; ?>" />
					  <? 
					  }
					}	                       
          unset($config["formulario"][1]["campos"][0]["evento"]);				  
					$linhaObj->GerarFormulario("formulario",$linha,$idioma);				          
				  } else {
					unset($config["formulario"][1]["campos"][0]["evento"]);
					 $config["formulario"][1]["campos"][1]["evento"] = "maxlength='14'";
					 $config["formulario"][1]["campos"][2]["evento"] = "maxlength='18'";
					$linhaObj->Set("config",$config);
					$linhaObj->GerarFormulario("formulario",$_POST,$idioma);
				  }
				  ?>
                  <div class="form-actions">
				    <?php if($url[4] == "editar" && !$sindicatos_associadas) { ?>
						<div class="alert alert-error fade in">
							<a href="javascript:void(0);" class="close" data-dismiss="alert">×</a>
							<strong><?= $idioma['sem_premissao_sindicatos']; ?></strong>
						</div>
					<?php } else { ?>
            ...
            if ($campo["botao_hide"]){					
              if ($campo['tipo'] == 'select') { ?>
                ...
                if (aux_d == 'cnpj'){
                  ...
                  $('#div_form_<?= $campo["iddiv7"]; ?>').show();
                  $('#div_form_<?= $campo["iddiv8"]; ?>').show();
                  $('#div_form_<?= $campo["iddiv9"]; ?>').show();

                  for (var i = 0; i < regras.length; i++){
                    if( regras[i] == 'required,form_<?= $campo["iddiv"]; ?>,<?= $idioma["cpf_vazio"] ?>'
                      || regras[i] == 'required,form_<?= $campo["iddiv3"]; ?>,<?= $idioma["rg_vazio"] ?>'
                      || regras[i] == 'required,form_<?= $campo["iddiv4"]; ?>,<?= $idioma["orgao_emissor_vazio"] ?>')
                      regras.splice(i, 1); 
                  }
                  for (var i = 0; i < regras.length; i++){
                ...
                $('#div_form_cnh').show();
                $('#div_form_categoria').show();
                $('#div_form_data_primeira_habilitacao').show();
                $('#div_cnh_data_emissao').show();
                $('#div_data_validade').show();
                ...
                $('#<?= $campo["id"]; ?>').change(function() {
                  ...
                  if (aux_d == 'cpf'){                    
                    ...
                    $('#div_form_<?= $campo["iddiv6"]; ?>').show("fast");

                    $('#div_form_<?= $campo["iddiv2"]; ?>').hide("fast");
                    $('#div_form_<?= $campo["iddiv7"]; ?>').hide("fast");
                    $('#div_form_<?= $campo["iddiv8"]; ?>').hide("fast");
                    $('#div_form_<?= $campo["iddiv9"]; ?>').hide("fast");
                    ...
                    $('#div_form_<?= $campo["iddiv7"]; ?>').show("fast");
                    $('#div_form_<?= $campo["iddiv8"]; ?>').show("fast");
                    $('#div_form_<?= $campo["iddiv9"]; ?>').show("fast");
                    ...
                    $('#div_form_<?= $campo["iddiv6"]; ?>').hide("fast");   
    ...
    <script type="text/javascript">
      $(document).ready(function(){		                
        ...
        /*if(<?=intval($linha["idpais"])?> > 0)
          $("#form_idpais").trigger("addItem",[{"title": "<?=$linha["pais"]?>", "value": "<?=$linha["idpais"]?>"}]);			
