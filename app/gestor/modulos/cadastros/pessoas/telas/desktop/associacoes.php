  ...
  <section id="global">
    <div class="page-header">
      ...
      <ul class="breadcrumb">
        ...
        <li><?php echo $linha["nome"]; ?> <span class="divider">/</span></li>
            <li class="active"><?php echo $idioma["associacoes"]; ?></li>
          ...
          <?php incluirTela("inc_menu_edicao",$config,$usuario); ?>
                ...
                <form class="well" method="post">
                  <p><?= $idioma["form_associar"]; ?></p>
                                <p><?= $idioma["form_pessoas"]; ?></p>
                <?php if($perfil["permissoes"][$url[2]."|7"]) { ?>
                                <select id="pessoas" name="pessoas"></select>
                                <br />
                                <?php
                                /*
                                    ?>
                                    <p><?= $idioma["form_tipo_associacao"]; ?></p>
                                    <select id="tipo_associacao" name="tipo_associacao">
                                      <?php foreach($tiposAssociacoesArray as $ind => $tipoAssociacao) { ?>
                                          <option value="<?php echo $tipoAssociacao["idtipo"]; ?>"><?php echo $tipoAssociacao["nome"]; ?></option>
                                        <?php } ?>
                                    </select>
                                    <br />
                                    <br />
                  ...
                  <input type="hidden" id="acao" name="acao" value="associar_pessoa">
                                ...
                                <br />
                                <p><?= $idioma["form_tipo_associacao"]; ?></p>
                                <select id="tipo_associacao" name="tipo_associacao" disabled="disabled"></select>
                                <br />
                                <br />
                            ...
                            <br />
                <form method="post" id="remover_associacao_pessoa" name="remover_associacao_pessoa">
                  <input type="hidden" id="acao" name="acao" value="remover_associacao_pessoa">
                    ...
                    <?php foreach($associacoesArray as $ind => $associacao) { ?>
                                            <tr>
                                                <td><?php echo $associacao["idpessoa_associacao"]; ?></td>
                                                <td><?php echo $associacao["nome"]; ?></td>
                                                <td><?php echo $associacao["documento"]; ?></td>
                                                <td><?php echo $associacao["tipo"]; ?></td>
                                                <td>
                                                  <?php if($perfil["permissoes"][$url[2]."|8"]) { ?>
                                                   <a href="javascript:void(0);" class="btn btn-mini" data-original-title="<?= $idioma["btn_remover"]; ?>" data-placement="left" rel="tooltip" onclick="remover(<?php echo $associacao["idpessoa_associacao"]; ?>)"><i class="icon-remove"></i></a>
                                                  <?php } else { ?>
    ...
    $(document).ready(function(){
      $("#pessoas").fcbkcomplete({
        json_url: "/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/<?= $url[3]; ?>/json/associar_pessoas",
        addontab: true,
        height: 10,
        maxshownitems: 10,
        cache: true,
    ...
    function remover(id) {
      ...
      if(confirma) {
        ...
        document.getElementById("remover_associacao_pessoa").submit();
      }
    }
  </script>
</div>
