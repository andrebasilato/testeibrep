                ...
                <tbody>
				  <?php if(count($sindicatos) > 0) { ?>
					<?php foreach($sindicatos as $ind => $sindicato) { ?>
                      <tr>
                        <td><?php echo $sindicato["idsindicato"]; ?> - <?php echo $sindicato["nome_abreviado"]; ?></td>
                        <td>
						  <?php if($perfil["permissoes"][$url[2]."|13"]) { ?>
                          <a href="javascript:void(0);" class="btn btn-mini" data-original-title="<?= $idioma["btn_remover"]; ?>" data-placement="left" rel="tooltip" onclick="remover(<?php echo $sindicato["idpessoa_sindicato"]; ?>)"><i class="icon-remove"></i></a>
                          <?php } else { ?>
