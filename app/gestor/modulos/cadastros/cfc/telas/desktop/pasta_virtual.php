...
<div class="container-fluid">
  <section id="global">
    ...
    <ul class="breadcrumb">
      ...
      <li><a href="/<?php echo $url[0]; ?>/<?php echo $url[1]; ?>/cfc"><?php echo $idioma["pagina_titulo"]; ?></a> <span class="divider">/</span></li>
      <li><?php echo $idioma["nav_escola"]; ?> <?php echo $linha["nome"]; ?> </li>
            ...
            <section id="pastavirtual">
				...
				<?php if($mensagem["erro"]) { ?>
				  <div class="alert alert-error">
					...
					<?= $idioma[$mensagem["erro"]].' - '.$mensagem["erro"]; ?>
				  </div>
			   ...
			   <div id="pastavirtual_ancora_div">
				...
				<form method="post" action="" style="padding-top:15px;" enctype="multipart/form-data">
					...
					<table style="width:500px;" cellpadding="5" cellspacing="0" class="table table-bordered table-condensed ">
						<tr>
							<td bgcolor="#F4F4F4"><strong><?=$idioma["documentos_escola_arquivo"];?></strong></td>
				...
				<table width="700" cellpadding="5" cellspacing="0" class="table table-bordered table-condensed tabelaSemTamanho">
					<tr>
						<td bgcolor="#F4F4F4"><strong><?=$idioma["documentos_escola_arquivo"];?></strong></td>
						<td bgcolor="#F4F4F4"><strong><?=$idioma["documentos_escola_opcoes"];?></strong></td>
					</tr>
					...
					<tr>
						...
						<?php if ($documento["arquivo_nome"]) { ?>
							<?php if (strpos($documento['arquivo_tipo'],'image') !== false) { ?>
								<a href="/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/<?= $url[3]; ?>/<?= $url[4]; ?>/visualizararquivo/<?= $documento["iddocumento"]; ?>" class="fancybox btn btn-mini" rel="gallery" title="<?= $documento["tipo"].' ('.$documento["arquivo_nome"].')'; ?>"><i class="icon-picture"></i><?=$idioma["documentos_escola_visualizar"];?></a>
							  <?php } ?>
							  <a href="/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/<?= $url[3]; ?>/<?= $url[4]; ?>/downloadarquivo/<?= $documento["iddocumento"]; ?>" class="btn btn-mini" rel="tooltip" data-original-title="<?php echo $idioma["documentos_escola_download"]; ?>" data-placement="left"><i class="icon-download-alt"></i></a>
							<?php } else { ?>
							  <a href="javascript:enviarArquivo(<?= $documento["iddocumento"]; ?>);" class="btn btn-mini" rel="tooltip" data-original-title="<?php echo $idioma["documentos_escola_enviar"]; ?>" data-placement="left"><i class="icon-upload"></i><?=$idioma["documentos_escola_enviar"];?></a>
							<?php } ?>
							  <? if($linhaObj->verificaPermissao($perfil["permissoes"], $url[2]."|6", false)) { ?>
