...
<div class="container-fluid">
                    ...
                    <div class="tab-content">
                        <div class="tab-pane active" id="tab_editar">
                            ...
                            <form class="well" method="post">
                                <p><?= $idioma["form_estado_cidade"]; ?></p>
                                <?php if ($perfil["permissoes"][$url[2] . "|11"]) { ?>
                                    <select id="estados_cidades" name="estados_cidades"></select>
                                    <br/>
                                    <br/>
                                    <input type="hidden" id="acao" name="acao" value="associar_estado_cidade">
                            ...
                            <form method="post" id="remover_estado_cidade" name="remover_estado_cidade">
                                <input type="hidden" id="acao" name="acao" value="remover_estado_cidade">
                            ...
                            <table class="table table-striped tabelaSemTamanho">
                                ...
                                <tbody>
                                <?php if (count($estadosCidades) > 0) { ?>
                                    <?php foreach ($estadosCidades as $ind => $var) { ?>
                                        <tr>
                                            <td><?= $var['estado'] . ' - ' . $var['cidade']; ?></td>
                                            <td>
                                                <?php if ($perfil["permissoes"][$url[2] . "|12"]) { ?>
                                                    <a href="javascript:void(0);" class="btn btn-mini"
                                                       ...
                                                       onclick="remover(<?= $var["idescola_estado_cidade"]; ?>)"><i
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            $("#estados_cidades").fcbkcomplete({
                json_url: "/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/<?= $url[3]; ?>/json/associar_estados_cidades",
                addontab: true,
                height: 10,
                maxshownitems: 10,
                cache: true,
        ...
        function remover(id) {
            ...
            if (confirma) {
                ...
                document.getElementById("remover_estado_cidade").submit();
            }
        }
    </script>
</div>
