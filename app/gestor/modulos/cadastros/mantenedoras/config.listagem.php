...
$config["listagem"] = array(
  array(
	"id" => "idmantenedora",
	"variavel_lang" => "tabela_idmantenedora", 
	"tipo" => "php", 
	"coluna_sql" => "idmantenedora", 
	"valor" => '
			$diferenca = dataDiferenca($linha["data_cad"], date("Y-m-d H:i:s"), "H");
			if($diferenca > 24) {
				return "<span title=\"$diferenca\">".$linha["idmantenedora"]."</span>";
			} else {
				return "<span title=\"$diferenca\">".$linha["idmantenedora"]."</span> <i class=\"novo\"></i>";
			}
			', 	
	"busca" => true,
  ...
  array(
	...
	"coluna_sql" => "telefone", // Nome da coluna no banco de dados
  ...
  */									  
  ...
  array(
	...
	"tipo" => "php", 
	"valor" => 'return "<a class=\"btn dropdown-toggle btn-mini\" data-original-title=\"".$idioma["tabela_opcoes_tooltip"]."\" href=\"/".$this->url["0"]."/".$this->url["1"]."/".$this->url["2"]."/".$linha["idmantenedora"]."/opcoes\" data-placement=\"left\" rel=\"tooltip facebox\">".$idioma["tabela_opcoes"]."</a>"',
	"busca_botao" => true,
	"tamanho" => "80"
  ) 			
);						   
