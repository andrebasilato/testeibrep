      ...
      <div class="tab-content">
		...
		<h2 class="tituloOpcao"><?= $idioma["titulo_opcao"]; ?></h2>
          ...
          <form method="post" action="" class="form-horizontal">
            <input name="acao" type="hidden" value="cancelarmatricula" />
            <div class="control-group">
            <p>
              <? printf($idioma["usuario_selecionado"], $aluno->nome, $info->idmatricula); ?>
              <br />
              <br />
            ...
            <div class="form-actions">
              <a type="submit" id="enviar" href="<?php echo Request::url('0-3', '/') . Request::url(4) .'/removerpagina/'.Request::url('6'); ?>/remove" class="btn btn-primary" style="color: #FFFFFF"><?= $idioma["remover"]; ?>	</a>&nbsp;
              <button type="reset" class="btn" onclick="MM_goToURL('parent','/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/<?= $url[3]; ?>/diplomas');"><?= $idioma["cancelar"]; ?></button>
            </div>
          </form>
        </div>
      </div>
