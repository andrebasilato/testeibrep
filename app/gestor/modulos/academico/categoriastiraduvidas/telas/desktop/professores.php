...
<div class="container-fluid">
              		...
              		<div class="tab-content">
                		<div class="tab-pane active" id="tab_editar">
    						...
    						<form class="well" method="post">
                                ...
                                <p><?= $idioma["form_professores"]; ?></p>
								<?php if($perfil["permissoes"][$url[2]."|5"]) { ?>    
                                <select id="professores" name="professores"></select>
                                <br />
                                <br />
    							<input type="hidden" id="acao" name="acao" value="associar_professor">
                                <input type="submit" class="btn" value="<?= $idioma["btn_adicionar"]; ?>" />
                                <?php } else { ?>
                                <select id="professores" name="professores" disabled="disabled"></select>
                                <br />
                                <br />
                            ...
                            <br />
     						<form method="post" id="remover_associacao_professor" name="remover_associacao_professor">
    							<input type="hidden" id="acao" name="acao" value="remover_associacao_professor">
        						...
        						<tbody>
                                    <?php if(count($associacoesArray) > 0) { ?>
										<?php foreach($associacoesArray as $ind => $associacao) { ?>
                                            <tr>
                                                <td><?= $associacao["idcategoria_professor"]; ?></td>
                                                <td><?= $associacao["nome"]; ?></td>
                                                <td>
                                                  <?php if($perfil["permissoes"][$url[2]."|6"]) { ?>
                                                  <a href="javascript:void(0);"  class="btn btn-mini" data-original-title="<?= $idioma["btn_remover"]; ?>" data-placement="left" rel="tooltip" onclick="remover(<?= $associacao["idcategoria_professor"]; ?>)"><i class="icon-remove"></i></a>
                                                  <?php } else { ?>
    ...
    <script type="text/javascript">
		$(document).ready(function(){                
			$("#professores").fcbkcomplete({
				json_url: "/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/<?= $url[3]; ?>/json/associar_professores",
				addontab: true,
				height: 10,
				maxshownitems: 10,
				cache: true,
		...
		function remover(id) {
			...
			if(confirma) {
				...
				document.getElementById("remover_associacao_professor").submit();
			} 
		}
	</script>
</div>
