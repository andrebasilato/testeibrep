...
if ('removerpagina' == Request::url(5)) {
    $certified = new Certificados;
    $page = $certified->getPageInfo(Request::url(4));
    if ('id' == Request::url(6)) {
        $certified->set('idusuario', $usuario['idusuario']);
        ...
        if ($linha) {
            switch ($url[4])
            {
               case 'editar' :
                    ...
                    break;
              case 'paginas' :
                    $linhaObj->verificaPermissao($perfil['permissoes'], Request::url(3) . '|3');
              ...
              case 'imagens' :
                   if ('salvar_imagem' == Request::post('acao')) {
					...
					if (Request::url(7) && 'baixar_imagem' == Request::url(7)) {
						...
						$sql = "select * from certificados_midias where idcertificado_midia='".$url[5]."';";
						$imagem = $linhaObj->retornarLinha($sql);
						$arquivo = $_SERVER["DOCUMENT_ROOT"]."/storage/certificados_midias/".$url[3]."/".$imagem["arquivo_servidor"];
						...
						header('Content-Disposition: attachment; filename="'. basename($imagem["arquivo_nome"]).'"');
