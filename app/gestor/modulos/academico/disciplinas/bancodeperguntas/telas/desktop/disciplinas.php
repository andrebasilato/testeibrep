            ...
            <div class="tab-content">
              <div class="tab-pane active" id="tab_editar">
                ...
                <form class="well" method="post" id="form">
                    ...
                    <br />
                    <br />
                    <input type="hidden" id="acao" name="acao" value="associar_disciplinas">
              ...
              <table class="table table-striped tabelaSemTamanho">
                  <thead>
                      <tr>
                        <th><?= $idioma["listagem_disciplina"]; ?></th>
                        <th width="60"><?= $idioma["listagem_opcoes"]; ?></th>
                      </tr>
                  </thead>
                  <tbody>
                      <?php if(count($disciplinas) > 0) { ?>
                          <?php foreach($disciplinas as $associacao) { ?>
                              <tr>                                                
                                  <td><?php echo $associacao["nome"]; ?></td>
                                  <td>
                                    <?php if($perfil["permissoes"][$url[2]."|6"]) { ?>
                                    <a href="javascript:void(0);" class="btn btn-mini" data-original-title="<?= $idioma["btn_remover"]; ?>" data-placement="left" rel="tooltip" onclick="remover(<?php echo $associacao["iddisciplina_pergunta"]; ?>)"><i class="icon-remove"></i></a>
                                    <?php } else { ?>
    ...
    <script type="text/javascript">
		$(document).ready(function(){                
			$("#disciplinas").fcbkcomplete({
				json_url: "/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/<?= $url[3]; ?>/json/associar_disciplina",
				addontab: true,
				height: 10,
				maxshownitems: 10,
				cache: true,
