...
<div class="container-fluid">
  	...
  	<div class="row-fluid">
  		<div class="span12">
        	<div class="box-conteudo">
            	...
            	<h2 class="tituloEdicao"><?= $linha["oferta"]; ?> </h2>
              		...
              		<div class="tab-content">
                        ...
                        <h2 class="tituloOpcao"><?= $idioma["titulo_opcao"]; ?></h2>
    						...
    						<form class="well" method="post">
                                ...
                                <p><?= $idioma["form_escolas"]; ?></p>
								<?php if($perfil["permissoes"][$url[2]."|9"]) { ?>    
                                <select id="escolas" name="escolas"></select>
                                <br />
                                <br />
                            ...
                            <br />
     						<form method="post" id="remover_associacao_escola" name="remover_associacao_escola">
    							<input type="hidden" id="acao" name="acao" value="remover_associacao_escola">
        						...
        						<tbody>
                                    <?php if(count($associacoesArray) > 0) { ?>
										<?php foreach($associacoesArray as $ind => $associacao) { ?>
                                            <tr>
                                                <td><?php echo $associacao["idoferta_curso_escola"]; ?></td>
                                                <td><?php echo $associacao["nome_fantasia"]; ?></td>
                                                <td>
                                                  <?php if($perfil["permissoes"][$url[2]."|10"]) { ?>
                                                  <a href="javascript:void(0);"  class="btn btn-mini" data-original-title="<?= $idioma["btn_remover"]; ?>" data-placement="left" rel="tooltip" onclick="remover(<?php echo $associacao["idoferta_curso_escola"]; ?>)"><i class="icon-remove"></i></a>
                                                  <?php } else { ?>
    ...
    <script type="text/javascript">
		$(document).ready(function(){                
			$("#escolas").fcbkcomplete({
				json_url: "/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/<?= $url[3]; ?>/<?= $url[4]; ?>/<?= $url[5]; ?>/json/associar_escolas",
				addontab: true,
				height: 10,
				maxshownitems: 10,
				cache: true,
		...
		function remover(id) {
			...
			if(confirma) {
				...
				document.getElementById("remover_associacao_escola").submit();
			} 
		}
	</script>
</div>
