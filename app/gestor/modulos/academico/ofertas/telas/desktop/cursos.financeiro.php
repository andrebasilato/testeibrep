			...
			<div class="ava-conteudo"> 
              <div class="tab-pane active" id="tab_editar">
                ...
                <form method="post" enctype="multipart/form-data" class="form-horizontal">
                 	<input name="acao" type="hidden" value="salvar_financeiro_curso" />
                 	<input name="idoferta_curso" type="hidden" value="<?= $linha["idoferta_curso"] ?>" />
                 	<input name="data_inicio_aula" type="hidden" value="<?= $linha["data_inicio_aula"] ?>" />
                	<legend></legend>
                	<div class="control-group">
	                	<label class="control-label" for="form_nome">
	                		<strong>* <?= $idioma['possui_financeiro']; ?></strong>
	                	</label>
	                	<div class="controls">
	                		<select id="possui_financeiro" name="possui_financeiro" class="span2">
								<option value="S" 
									<?= $linha['possui_financeiro'] == 'S' ? "selected" : "" ?>>
									<?= $idioma['opcao_sim'] ?>
								</option>									
								<option value="N"
									<?= $linha['possui_financeiro'] == 'N' ? "selected" : "" ?>>
									<?= $idioma['opcao_nao'] ?>
								</option>
							</select>	                		
	                	</div>
                	...
                	<div class="form-actions">
                    	...
                    	<input type="reset" class="btn" 
                    		   onclick="MM_goToURL('parent','/<?= $url[0] ?>/<?= $url[1] ?>/<?= $url[2] ?>/<?= $url[3] ?>/<?= $url[4] ?>');" 
                    		   value="Cancelar" />
                  	</div>             
                </form>
              </div>
