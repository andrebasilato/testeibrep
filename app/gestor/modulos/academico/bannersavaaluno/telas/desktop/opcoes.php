    ...
    <li>
     ...
     <li>
	  ...
	  <?php if($linhaObj->verificaPermissao($perfil["permissoes"], $url[2]."|10", NULL)){ ?>
        <a href="/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/<?php echo $linha["idbanner"]; ?>/cursos"> <i class="icon-briefcase"></i> <? echo $idioma["cursos"]; ?></a>
      <?php } else { ?>
        <a href="javascript:void(0)" data-original-title="<?= $idioma["opcao_permissao"] ?>"  data-placement="left" rel="tooltip" style="color:#999;"> <i class="icon-briefcase icon-white"></i> <? echo $idioma["cursos"]; ?></a>
      <?php } ?>
    </li>
    <li>
