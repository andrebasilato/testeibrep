...
if($_POST["acao"] == "gerar_contrato") {
    $matriculaObj->verificaPermissao($perfil["permissoes"], $url[2]."|13");
  ...
  $matriculaObj->verificaPermissao($perfil["permissoes"], $url[2]."|15");
  ...
  $matriculaObj->verificaPermissao($perfil["permissoes"], $url[2]."|14");
  ...
  $matriculaObj->verificaPermissao($perfil["permissoes"], $url[2]."|16");
...
if($url[6]) {
    switch ($url[6]) {
        ...
        case "contrato":
            ...
            if ($url[8] == 'pendentes') {
                $contrato = $matriculaObj->retornarContratoPendente(intval($url[7]));
            } else {
        ...
        case "contratopdf":
            ...
            $pasta = ($contrato['contrato_pendente'] == 'S') ? 'matriculas_contratos_pendentes' : 'matriculas_contratos';
...
$matricula['contratos'] = array_merge($matricula['contratos'], $contratosPendentes);
...
//$existeDevedorSolidario = $matriculaObj->existeDevedorSolidario();
