...
if ($_POST["acao"] == "gerar_declaracao") {
    ...
    $matriculaObj->verificaPermissao($perfil["permissoes"], $url[2]."|18");
...
if($url[6]) {
    switch ($url[6]) {
        ...
        case "declaracaopdf":
          ...
          //Altera de link externo para link interno das declarações já geradas, evitando erros com imagens no mPDF
          $search = array("http://" . $_SERVER["SERVER_NAME"] . "/storage/", "https://" . $_SERVER["SERVER_NAME"] . "/storage/");
          $replace = $_SERVER['DOCUMENT_ROOT'] . "/storage/";
          $saida = str_ireplace($search, $replace, $saida);
