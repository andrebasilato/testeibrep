...
if($_POST["acao"] == "adicionar_documento") {
  $matriculaObj->verificaPermissao($perfil["permissoes"], $url[2]."|10");
  ...
  $matriculaObj->verificaPermissao($perfil["permissoes"], $url[2]."|12");
  if($matricula["situacao"]["visualizacoes"][9]) {
    $matriculaObj->Set("id", $matricula["idmatricula"]);
  ...
  if($remover["sucesso"]){
	...
	$matriculaObj->verificaPermissao($perfil["permissoes"], $url[2]."|11");

	if ($matricula["situacao"]["visualizacoes"][8]) {
		$matriculaObj->Set("id", $matricula["idmatricula"]);
...
$tiposDocumentos = $tiposDocumentosObj->set('idmatricula', intval($url[3]))
										->retornarTodosComObrigatorio($matricula["idsindicato"], $matricula["curso"]["idcurso"]);
