...
<div class="container-fluid">
    ...
    <div class="row-fluid">
        <div class="span12">
            <div class="box-conteudo">
                ...
                <? } ?>
                <? if ($matriculaObj->verificaPermissao($perfil["permissoes"], $url[2] . "|2", NULL)) { ?>
                    <span class="pull-right" style="padding-top:3px; color:#999">
                <a href="/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/novamatricula" class="btn  btn-primary"><i
                            class="icon-plus icon-white"></i> <?= $idioma["nav_cadastrar"]; ?></a>
              </span>
                <? } ?>
                ...
                <?php $matriculaObj->GerarTabela($dadosArray, $_GET["q"], $idioma); ?>
                        ...
                        <div class="inline-inputs">
                            ...
                            <form action="" method="get" id="formQtd">
                                <? if ($_GET["buscarpor"] && $_GET["buscarem"]) { ?>
                                    <input name="buscarpor" type="hidden" id="buscarporQtd"
                                           value="<?= $_GET["buscarpor"]; ?>">
                                    <input name="buscarem" type="hidden" id="buscaremQtd"
                                           value="<?= $_GET["buscarem"]; ?>">
                                <? } ?>
                                <? if (is_array($_GET["q"])) {
                                    foreach ($_GET["q"] as $ind => $valor) {
                                        ?>
                                        <input id="q[<?= $ind ?>]" type="hidden" value="<?= $valor; ?>"
                                               name="q[<?= $ind ?>]"/>
                                    <? }
                                } ?>
                                <input id="cmp" type="hidden" value="<?= $matriculaObj->Get("ordem_campo"); ?>"
                                       name="Cmp"/>
                                <? if ($_GET["ord"]) { ?>
                                    <input id="ord" type="hidden" value="<?= $_GET["ord"]; ?>" name="ord"/>
                                <? } ?>
                                <input name="qtd" type="text" class="span1" id="qtd" maxlength="4"
                                       value="<?= $matriculaObj->Get("limite"); ?>"/>
                                <a href="javascript:document.getElementById('formQtd').submit();"
                                   class="btn small"><?= $idioma["exibir"]; ?></a>
                            </form>
                        </div>
                    </div>
                </div>
                <? if ($matriculaObj->Get("paginas") > 1) { ?>
                    ...
                    <div class="pagination">
                        <ul><?= $matriculaObj->GerarPaginacao($idioma); ?></ul>
                    </div>
                <? } ?>
    ...
    <script language="javascript" type="text/javascript">
        jQuery(document).ready(function ($) {
            $(".select231").select2({dropdownCssClass: 'bigdrop'});
