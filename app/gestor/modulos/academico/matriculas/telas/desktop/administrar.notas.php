                ...
                <div class="row-fluid">
                    ...
                    <div class="span12">
                        ...
                        <section id="notasmatricula">
                            ...
                            <?
                            if($matricula["situacao"]["visualizacoes"][25] && $matriculaObj->verificaPermissao($perfil["permissoes"], $url[2]."|27",false)) {
                                ?>
                                                                ...
                                                                <? if(!$nota["idprova"] && !$nota["id_solicitacao_prova"]){ ?>
                                                                    <table width="100%" border="0" cellspacing="0" cellpadding="5" style="margin-bottom:0px">
                                                                        <tr>
                                                                            ...
                                                                            <?php/*<td><input name="modificar_nota[<?= $nota["iddisciplina"]; ?>][<?= $nota["idmatricula_nota"]; ?>]" class="check_remover_nota" type="checkbox" value="<?= $nota["idmatricula_nota"]; ?>" /></td>*/?>
                                                                        </tr>
                                                                    </table>
                                                                <? } else { ?>
                            ...
                            <?
                            if($matricula["situacao"]["visualizacoes"][24] && $matriculaObj->verificaPermissao($perfil["permissoes"], $url[2]."|23",false)) {
                                ?>
                            ...
                            <?php if($matricula["situacao"]["visualizacoes"][23] && $matriculaObj->verificaPermissao($perfil["permissoes"], $url[2]."|22",false)) { ?>
                            ...
                            <?php } ?>
                            <?php /*?><form class="form-inline" method="post" onsubmit="return validaNotas();">
        <input name="acao" type="hidden" value="lancar_notas" /><?php */?>
                            <br />
                            <table cellpadding="5" cellspacing="0" class="table table-bordered table-condensed tabelaSemTamanho" <?php if($margin) { ?>style="margin-top:<?php echo $margin; ?>px;<?php } ?>">
                                <tr>
                                ...
                                <? foreach($matricula["disciplinas"] as $ind => $disciplina) { ?>
                                    <tr>
                                        ...
                                        <? } ?>
                                        <? /*if($matricula["situacao"]["visualizacoes"][23] && $matriculaObj->verificaPermissao($perfil["permissoes"], $url[2]."|22",false)) { ?>
                    <td align="center" style="text-align:center;"><input type="checkbox" name="ignorar[<?= $ind; ?>][<?= $disciplina["iddisciplina"]; ?>]" id="ignorar_<?= $ind; ?>_<?= $disciplina["iddisciplina"]; ?>" checked="checked" onclick="apagarNota(this,'nota_<?= $ind; ?>_<?= $disciplina["iddisciplina"]; ?>');" value="1" <?php if (!$disciplina['situacao']['lancar_nota']) { ?> disabled="disabled" <?php } ?> /></td>
					<?php if ($disciplina['situacao']['lancar_nota']) { ?>
                        <td style="text-align:center">
                            <input class="span1 notasMatricula" type="text" name="nota[<?= $ind; ?>][<?= $disciplina["iddisciplina"]; ?>]" id="nota_<?= $ind; ?>_<?= $disciplina["iddisciplina"]; ?>" onblur="marcarDesmarcarIgnorar(this,'ignorar_<?= $ind; ?>_<?= $disciplina["iddisciplina"]; ?>');" maxlength="5" >
						</td>
						<td style="text-align:center">
                            <select name="tipo_nota[<?= $ind; ?>][<?= $disciplina["iddisciplina"]; ?>]" class="span2 tiposNotasMatricula">
                                <?php foreach ($tiposNotas as $tipo) { ?>
                                    <option value="<?php echo $tipo['idtipo']; ?>"><?php echo $tipo['nome']; ?></option>
                                <?php } ?>
                            </select>
						...
						<td style="text-align:center">
                            <select name="modelo_nota[<?= $ind; ?>][<?= $disciplina["iddisciplina"]; ?>]" class="span2 modelosNotasMatricula">
                                <?php foreach ($modelosProvas as $modelo) { ?>
                                    <option value="<?php echo $modelo['idmodelo']; ?>"><?php echo $modelo['nome']; ?></option>
                                <?php } ?>
                            </select>
				...
				<? }*/ ?>
                                ...
                                <tr>
                                    ...
                                    <? /*if($matricula["situacao"]["visualizacoes"][23] && $matriculaObj->verificaPermissao($perfil["permissoes"], $url[2]."|22",false)) { ?>
                <td style="text-align:right;" colspan="3"><input type="submit" name="button" id="button" value="<?= $idioma["notas_btn_salvar"]; ?>" class="btn btn-mini "/></td>
			<? }*/ ?>
                                </tr>
                                <? if($matricula["situacao"]["visualizacoes"][25] && $matriculaObj->verificaPermissao($perfil["permissoes"], $url[2]."|27",false)) { ?>
                                    <tr>
...
<script>
    ...
    /*function marcarDesmarcarIgnorar(nota, ignorar) {
        var valor = nota.value;
        if(valor) {
          valor = valor.replace(",",".");
          valor = parseFloat(valor);
          nota.value = number_format(valor, 2, ',', '');
          document.getElementById(ignorar).checked = false;
        } else {
          document.getElementById(ignorar).checked = true;
        }
    }*/

    /*function apagarNota(check, nota) {
        if(check.checked) {
