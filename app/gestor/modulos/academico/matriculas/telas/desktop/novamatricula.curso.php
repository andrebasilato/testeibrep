...
<div class="container-fluid">
    <section id="global">
        <div class="page-header"><h1><?= $idioma["pagina_titulo"]; ?>
                &nbsp;<small><?= $idioma["pagina_subtitulo"]; ?></small></h1></div>
        <ul class="breadcrumb">
                    ...
                    <div class="span12">
                        <section id="formulario_cpf">
                                ...
                                <form style="border-bottom:1px #F4F4F4 solid; padding:15px;" onsubmit="return false;">
                                    ...
                                    <select name="selecionaOferta" id="selecionaOferta" class="inputGrande">
                                        ...
                                        <? foreach ($ofertas as $oferta) { ?>
                                            <option <? if ($oferta["idoferta"] == $url[4]) { ?> selected="selected" <? } ?>
                                                    value="/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/<?= $url[3]; ?>/<?= $oferta["idoferta"]; ?>"><?= $oferta["nome"]; ?></option>
                                        <? } ?>
                                    </select>
                                    <br/>
                                    <br/>
                                    <a type="button" class="btn"
                                       onclick="mudarUrl('selecionaOferta','parent',1)"><?= $idioma["ver_cursos"]; ?></a>
                                </form>
                            <? } elseif ($url[4] && !$url[5]) { ?>
                                <span class="pull-right"><small><a
                                                href="/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/<?= $url[3]; ?>"><?= $idioma["seleciona_outra_oferta"]; ?></a></small></span>
                                <legend><?= $idioma["form_oferta"]; ?></legend>
                                <h2 style="margin-bottom:15px; font-size:30px; line-height:35px;"><?= $_SESSION["matricula"]["oferta_nome"]; ?></h2>
                                <form style="border-bottom:1px #F4F4F4 solid; padding:15px;" onsubmit="return false;">
                                    ...
                                    <select name="selecionaCurso" id="selecionaCurso" class="inputGrande">
                                        ...
                                        foreach ($ofertaCursos as $ofertaCurso) { ?>
                                            <option <? if ($ofertaCurso["idoferta_curso"] == $url[5]) { ?> selected="selected" <? } ?>
                                                    value="/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/<?= $url[3]; ?>/<?= $url[4]; ?>/<?= $ofertaCurso["idoferta_curso"]; ?>"><?= $ofertaCurso["curso"]; ?></option>
                                            <?php
                                        }
                                        ?>
                                    </select>
                                    ...
                                    <br/>
                                    <a type="button" class="btn"
                                       onclick="mudarUrl('selecionaCurso','parent',1)"><?= $idioma["ver_escolas"]; ?></a>
                                </form>
                            <? } elseif ($url[4] && $url[5] && !$url[6]) { ?>
                                ...
                                <h2 style="margin-bottom:15px; font-size:30px; line-height:35px;"><?= $_SESSION["matricula"]["oferta_nome"]; ?></h2>
                                <span class="pull-right"><small><a
                                                href="/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/<?= $url[3]; ?>/<?= $url[4]; ?>"><?= $idioma["seleciona_outro_curso"]; ?></a></small></span>
                                    ...
                                    <section class="blocoDisponibilidade" id="escolas">
                                        ...
                                        foreach ($ofertaCursoEscolas as $ofertaCursoEscola) {
                                            ...
                                            <div class="span3 unidadeDisponibilidade">
                                                <a href="/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/<?= $url[3]; ?>/<?= $url[4]; ?>/<?= $url[5]; ?>/<?= $ofertaCursoEscola["idoferta_curso_escola"]; ?>#turmas"
                                                   class="btn" rel="tooltip"
                                                   title="<?= $idioma["iniciar_matricula"]; ?>"
                                                   style="color: #FFFFFF; border-color: #006DCC; text-shadow: 0 -1px 0 rgba(0, 0, 0, 0.25); background: #006DCC;">
                            ...
                            <? } else { ?>
                                ...
                                <span class="pull-right"><small><a
                                                href="/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/<?= $url[3]; ?>/<?= $url[4]; ?>"><?= $idioma["seleciona_outro_curso"]; ?></a></small></span>
                                <legend><?= $idioma["form_oferta_curso"]; ?></legend>
                                <h2 style="margin-bottom:15px; font-size:30px; line-height:35px;"><?= $_SESSION['matricula']['curso_nome']; ?></h2>
                                <legend><?= $idioma["form_oferta_curso_escola"]; ?></legend>
                                            ...
                                            <div class="span3 unidadeDisponibilidade">
                                                <?php if ($escola["limite"] === '0' || ($escola["limite"] != "" && $ofertaTurma["total_turma"] >= $escola["limite"])) { ?>
                                                    <a href="javascript:void();"
                                                       onclick="javascript:alert('Turma completa.')" class="btn"
                                                       rel="tooltip" title="<?= $idioma["iniciar_matricula"]; ?>"
                                                       style="color: #FFFFFF; border-color: #696969; text-shadow: 0 -1px 0 rgba(0, 0, 0, 0.25); background: #696969;">
                                                        <div class="blocoLink"><?= $ofertaTurma["nome"]; ?>
                                                            (<?= $ofertaTurma["total_turma"]; ?>
                                                            /<?= $escola["limite"]; ?>)
                                                        </div>
                                                    </a>
                                                <?php } else { ?>
                                                    <a href="/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/<?= $url[3]; ?>/<?= $url[4]; ?>/<?= $url[5]; ?>/<?= $url[6]; ?>/<?= $ofertaTurma["idturma"]; ?>/aluno"
                                                       class="btn" rel="tooltip"
                                                       title="<?= $idioma["iniciar_matricula"]; ?>"
                                                       style="color: #FFFFFF; border-color: #006DCC; text-shadow: 0 -1px 0 rgba(0, 0, 0, 0.25); background: #006DCC;">
                                                        <?php if (is_null($escola["limite"])) { ?>
                                                            <div class="blocoLink"><?= $ofertaTurma["nome"]; ?><?php if (!is_null($ofertaTurma["total_turma"])) {
                                                                    echo '(' . $ofertaTurma["total_turma"] . ')';
                                                                } ?></div>
                                                        <?php } else { ?>
                                                            <div class="blocoLink"><?= $ofertaTurma["nome"]; ?>
                                                                (<?php echo (int)$ofertaTurma["total_turma"]; ?>
                                                                /<?= $escola["limite"]; ?>)
                                                            </div>
                                                        <?php } ?>
                                                    </a>
                                ...
                                <? } else { ?>
                                    <h5><?= $idioma['nenhuma_turma_disponivel']; ?></h5>
                                <? } ?>
                            <?php } ?>
                        </section>
                    </div>
