      ...
      <div class="box-conteudo" style="padding:35px;">
        <div class="row-fluid">
          <div class="span12">
              ...
              <h3 style="margin-bottom:15px; font-size:20px; line-height:25px; text-transform:uppercase;"><?= $escola["oferta"]; ?></h3>
              <legend><?= $idioma["label_curso"]; ?></legend>
              <h3 style="margin-bottom:15px; font-size:20px; line-height:25px; text-transform:uppercase;"><?= $escola["curso"]; ?></h3>
              <legend><?= $idioma["label_escola"]; ?></legend>
              <h3 style="margin-bottom:15px; font-size:20px; line-height:25px; text-transform:uppercase;"><?= $escola["nome_fantasia"]; ?></h3>
			  <legend><?= $idioma["label_turma"]; ?></legend>
              <h3 style="margin-bottom:15px; font-size:20px; line-height:25px; text-transform:uppercase;"><?= $turma["nome"]; ?></h3>
          </div>
          <div class="span12">
            <section id="formulario_cpf">
			  ...
			  <? if(($_GET["cpf"]) && (!count($matricula["erros"]))) { ?>
                <section id="formulario_cliente">
                  ...
                  <? } ?>
                  <form method="post" action="/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/<?= $url[3]; ?>/<?= $url[4]; ?>/<?= $url[5]; ?>/<?= $url[6]; ?>/<?= $url["7"]; ?>/atendente" onsubmit="return valida_form(this, regras)" enctype="multipart/form-data" class="form-horizontal">
                    <?
                    if($pessoa["idpessoa"]) {
                      ...
                      $matriculaObj->GerarFormulario("formulario_pessoas",$_POST,$idioma);
                    }
                    ?>
				...
				success: function(data) {
				   if(!data) {
						...
						for (i=0; i<array_regras.length; i++) {
							index = array_regras.indexOf("registered_email,form_email,<?php echo $idioma['email_cadastrado']; ?>");
							array_regras.splice(index, 1);
						}
					}
					return false;
			}});
