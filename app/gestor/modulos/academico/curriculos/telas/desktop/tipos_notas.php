  ...
  <div class="container-fluid">
            ...
            <div class="tab-content">
              <div class="tab-pane active" id="tab_editar">        
                ...
                <form class="well" method="post">
                  <p><?= $idioma["form_tipo_nota"]; ?></p>
                  <?php if($perfil["permissoes"][$url[2]."|17"]) { ?>    
                    <select id="tipos_notas" name="tipos_notas"></select>
                    <br />
                    <br />
                    <input type="hidden" id="acao" name="acao" value="associar_tipo_nota">
                    <input type="submit" class="btn" value="<?= $idioma["btn_adicionar"]; ?>" />
				  <?php } else { ?>
                    <select id="tipos_notas" name="tipos_notas" disabled="disabled"></select>
                    <br />
                    <br />
                ...
                <br />
                <form method="post" id="remover_tipo_nota" name="remover_tipo_nota">
                  <input type="hidden" id="acao" name="acao" value="remover_tipo_nota">
                ...
                <table class="table table-striped tabelaSemTamanho">
                  ...
                  <tbody>
					<?php if(count($tipos_notas) > 0) { ?>
					  <?php foreach($tipos_notas as $tipo) { ?>
                        <tr>
                          <td><?php echo $tipo["nome"]; ?></td>
                          <td>
                            <?php if($perfil["permissoes"][$url[2]."|18"]) { ?>
                              <a href="javascript:void(0);" class="btn btn-mini" data-original-title="<?= $idioma["btn_remover"]; ?>" data-placement="left" rel="tooltip" onclick="remover(<?php echo $tipo["idcurriculo_tipo"]; ?>)"><i class="icon-remove"></i></a>
                            <?php } else { ?>
    ...
    <script type="text/javascript">
	  $(document).ready(function(){           
		$("#tipos_notas").fcbkcomplete({
		  json_url: "/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/<?= $url[3]; ?>/json/associar_tipos_notas",
		  addontab: true,
		  height: 10,
		  maxshownitems: 10,
		  cache: true,
	  ...
	  function remover(id) {
		...
		if(confirma) {
		  ...
		  document.getElementById("remover_tipo_nota").submit();
		} 
	  }
	</script>
  </div>
