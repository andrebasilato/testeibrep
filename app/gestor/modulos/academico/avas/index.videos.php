...
if (isset($url[5])) {
  if('cadastrar' == $url[5]) {
    ...
    include("telas/".$config["tela_padrao"]."/formulario.videos.php");
    exit();
  } else {
    ...
    if ($linha) {
      switch($url[6]) {
        ...
        case "remover":
          ...
          include("telas/".$config["tela_padrao"]."/remover.videos.php");
        break;
        case "opcoes":
  ...
  include("telas/".$config["tela_padrao"]."/index.videos.php");
}