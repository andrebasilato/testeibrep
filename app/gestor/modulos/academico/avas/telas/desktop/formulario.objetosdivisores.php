...
<head>
    ...
    <link media="screen" href="/assets/plugins/colorpicker/ui.colorpicker.css" rel="stylesheet" type="text/css">
</head>
<body>
  ...
  <div class="container-fluid">
    <section id="global">
      ...
      <ul class="breadcrumb">
        ...
        <li><a href="/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/<?= $linha["idava"]; ?>/objetosdivisores"><?= $idioma["pagina_titulo_interno"]; ?></a> <span class="divider">/</span></li>
        <? if($url[5] == "cadastrar") { ?>
            ...
            <div class="ava-conteudo"> 
              ...
              <div class="tab-pane active" id="tab_editar">
                ...
                <form method="post" onsubmit="return validateFields(this, regras)" enctype="multipart/form-data" class="form-horizontal">
                  <input name="acao" type="hidden" value="salvar_objeto_divisor" />
                  <? if($url[6] == "editar") {
                    echo '<input type="hidden" name="'.$config["banco_objetos_divisores"]["primaria"].'" id="'.$config["banco_objetos_divisores"]["primaria"].'" value="'.$linha[$config["banco_objetos_divisores"]["primaria"]].'" />';
                    foreach($config["banco_objetos_divisores"]["campos_unicos"] as $campoid => $campo) {
                    ?>
                      <input name="<?= $campo["campo_form"]; ?>_antigo" id="<?= $campo["campo_form"]; ?>_antigo" type="hidden" value="<?= $linha[$campo["campo_banco"]]; ?>" />
                    <? 
                    }					  
                    $linhaObj->GerarFormulario("formulario_objetos_divisores",$linha,$idioma);				
                  } else {
                    $linhaObj->GerarFormulario("formulario_objetos_divisores",$_POST,$idioma);
                  }
                  ?>
                  <div class="form-actions">
                    <input type="submit" class="btn btn-primary" value="<?= $idioma["btn_salvar"]; ?>">&nbsp;
                    <input type="reset" class="btn" onclick="MM_goToURL('parent','/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/<?= $url[3] ?>/<?= $url[4] ?>');" value="<?= $idioma["btn_cancelar"]; ?>" />
                  </div>
                </form>	
              </div>
            </div>
    ...
    <script language="javascript" src="/assets/plugins/colorpicker/jq.color.js"></script>
	<script type="text/javascript">
      var regras = new Array();
      <?php
      foreach($config["formulario_objetos_divisores"] as $fieldsetid => $fieldset) {
      ...
      jQuery(document).ready(function($) { 
        ...
        foreach($config["formulario_objetos_divisores"] as $fieldsetid => $fieldset) {
          foreach($fieldset["campos"] as $campoid => $campo) {
			...
			if($campo["colorpicker"]){ ++$cont_color; ?>    
				var hideit = function(e, ui) { $(this).val('#'+ui.hex); $('.ui-colorpicker').css('display', 'none'); };
				$("#<?= $campo["id"]; ?>").colorpicker({ hide: hideit, submit: hideit });
				$("#mudar_valor_<?php echo $cont_color; ?>").click(function() { 
					document.getElementById('<?= $campo["id"]; ?>').value = $('#valor_final_cor_<?php echo $cont_color; ?>').val();   
					return false;
				});
			<? }
			if($campo["editor"]){	
