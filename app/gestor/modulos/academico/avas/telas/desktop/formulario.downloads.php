  ...
  <div class="container-fluid">
    <section id="global">
      ...
      <ul class="breadcrumb">
        ...
        <li><a href="/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/<?= $linha["idava"]; ?>/<?= $url[4]; ?>"><?= $idioma["pagina_titulo_interno"]; ?></a> <span class="divider">/</span></li>
        <? if($url[5] == "cadastrar") { ?>
          ...
          <div class="tabbable tabs-left">
            ...
            <div class="ava-conteudo"> 
              ...
              <?php if($url[5] != "cadastrar") { ?>
                ...
                <?php include("inc_submenu_downloads.php"); ?>
              <?php } ?>
              <div class="tab-pane active" id="tab_editar">
                ...
                <form method="post" onsubmit="return validateFields(this, regras)" enctype="multipart/form-data" class="form-horizontal">
                  <input name="acao" type="hidden" value="salvar_download" />
                  <? if($url[6] == "editar") {
                    echo '<input type="hidden" name="'.$config["banco_downloads"]["primaria"].'" id="'.$config["banco_downloads"]["primaria"].'" value="'.$linha[$config["banco_downloads"]["primaria"]].'" />';
                    foreach($config["banco_downloads"]["campos_unicos"] as $campoid => $campo) {
                    ?>
                      <input name="<?= $campo["campo_form"]; ?>_antigo" id="<?= $campo["campo_form"]; ?>_antigo" type="hidden" value="<?= $linha[$campo["campo_banco"]]; ?>" />
                    <? 
                    }	
					$config["formulario_downloads"][0]["campos"][4]["validacao"] = array("formato_arquivo" => "arquivo_invalido");
					$config["banco"] = $config["banco_downloads"];
					$config["formulario"] = $config["formulario_downloads"];
					$linhaObj->Set("config",$config);
					$linhaObj->GerarFormulario("formulario_downloads",$linha,$idioma);				
                  } else {
                    $linhaObj->GerarFormulario("formulario_downloads",$_POST,$idioma);
                  }
                  ?>
    ...
    <script type="text/javascript">
      ...
      <?php
      foreach($config["formulario_downloads"] as $fieldsetid => $fieldset) {
