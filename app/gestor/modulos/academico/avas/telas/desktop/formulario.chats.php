  ...
  <div class="container-fluid">
    <section id="global">
      ...
      <ul class="breadcrumb">
        ...
        <li><a href="/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/<?= $linha["idava"]; ?>/chats"><?= $idioma["pagina_titulo_interno"]; ?></a> <span class="divider">/</span></li>
        <? if($url[5] == "cadastrar") { ?>
          ...
          <div class="tabbable tabs-left">
            ...
            <div class="ava-conteudo">
              ...
              <?php if($url[5] != "cadastrar") { ?>
                ...
                <?php include("inc_submenu_chats.php"); ?>
              <?php } ?>
              <div class="tab-pane active" id="tab_editar">
                ...
                <form method="post" onsubmit="return validateFields(this, regras)" enctype="multipart/form-data" class="form-horizontal">
                  <input name="acao" type="hidden" value="salvar_chat" />
                  <? if($url[6] == "editar") {
                    echo '<input type="hidden" name="'.$config["banco_chats"]["primaria"].'" id="'.$config["banco_chats"]["primaria"].'" value="'.$linha[$config["banco_chats"]["primaria"]].'" />';
                    foreach($config["banco_chats"]["campos_unicos"] as $campoid => $campo) {
                    ?>
                      <input name="<?= $campo["campo_form"]; ?>_antigo" id="<?= $campo["campo_form"]; ?>_antigo" type="hidden" value="<?= $linha[$campo["campo_banco"]]; ?>" />
                    <?
                    }
                    $linhaObj->GerarFormulario("formulario_chats",$linha,$idioma);
                  } else {
                    $linhaObj->GerarFormulario("formulario_chats",$_POST,$idioma);
                  }
                  ?>
    ...
    <link rel="stylesheet" href="/assets/plugins/datetimepicker/jquery.datetimepicker.css" type="text/css">
	<script src="/assets/js/ajax.js"></script>
	<script type="text/javascript">
	var regras = new Array();
	<?php
	$config_formulario = $config['formulario_chats'];
	...
	<?
		foreach($config["formulario_chats"] as $fieldsetid => $fieldset) {
			foreach($fieldset["campos"] as $campoid => $campo) {
				if($campo["mascara"]){ ?>
