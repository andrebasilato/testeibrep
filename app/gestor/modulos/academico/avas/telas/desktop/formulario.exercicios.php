...
<body>
  ...
  <div class="container-fluid">
            ...
            <div class="ava-conteudo"> 
              ...
              <div class="tab-pane active" id="tab_editar">
                ...
                <form method="post" onsubmit="return validateFields(this, regras)" enctype="multipart/form-data" class="form-horizontal">
                  <input name="acao" type="hidden" value="salvar_exercicio" />
                  <? if($url[6] == "editar") {
                    echo '<input type="hidden" name="'.$config["banco_exercicios"]["primaria"].'" id="'.$config["banco_exercicios"]["primaria"].'" value="'.$linha[$config["banco_exercicios"]["primaria"]].'" />';
                    foreach($config["banco_exercicios"]["campos_unicos"] as $campoid => $campo) {
                    ?>
                      <input name="<?= $campo["campo_form"]; ?>_antigo" id="<?= $campo["campo_form"]; ?>_antigo" type="hidden" value="<?= $linha[$campo["campo_banco"]]; ?>" />
                    <? 
                    }					  
                    $linhaObj->GerarFormulario("formulario_exercicios",$linha,$idioma);				
                  } else {
                    $linhaObj->GerarFormulario("formulario_exercicios",$_POST,$idioma);
                  }
                  ?>
    ...
    <script type="text/javascript">
      ...
      <?php
      foreach($config["formulario_exercicios"] as $fieldsetid => $fieldset) {
