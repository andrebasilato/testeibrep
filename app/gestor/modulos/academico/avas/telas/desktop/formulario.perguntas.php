  ...
  <div class="container-fluid">
    <section id="global">
      ...
      <ul class="breadcrumb">
        ...
        <li><a href="/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/<?= $linha["idava"]; ?>/perguntas"><?= $idioma["pagina_titulo_interno"]; ?></a> <span class="divider">/</span></li>
        <? if($url[5] == "cadastrar") { ?>
          ...
          <div class="tabbable tabs-left">
            ...
            <div class="ava-conteudo"> 
              ...
              <?php if($url[5] != "cadastrar") { ?>
                ...
                <?php include("inc_submenu_perguntas.php"); ?>
              <?php } ?>
              <div class="tab-pane active" id="tab_editar">
                ...
                <form method="post" onsubmit="return validateFields(this, regras)" enctype="multipart/form-data" class="form-horizontal">
                  <input name="acao" type="hidden" value="salvar_pergunta" />
                  <? if($url[6] == "editar") {
                    echo '<input type="hidden" name="'.$config["banco_perguntas"]["primaria"].'" id="'.$config["banco_perguntas"]["primaria"].'" value="'.$linha[$config["banco_perguntas"]["primaria"]].'" />';
                    foreach($config["banco_perguntas"]["campos_unicos"] as $campoid => $campo) {
                    ?>
                      <input name="<?= $campo["campo_form"]; ?>_antigo" id="<?= $campo["campo_form"]; ?>_antigo" type="hidden" value="<?= $linha[$campo["campo_banco"]]; ?>" />
                    <? 
                    }					  
                    $linhaObj->GerarFormulario("formulario_perguntas",$linha,$idioma);				
                  } else {
                    $linhaObj->GerarFormulario("formulario_perguntas",$_POST,$idioma);
                  }
                  ?>
    ...
    <script type="text/javascript">
      ...
      <?php
      foreach($config["formulario_perguntas"] as $fieldsetid => $fieldset) {
