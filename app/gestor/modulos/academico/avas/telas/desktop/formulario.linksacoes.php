...
<body>
  ...
  <div class="container-fluid">
          ...
          <div class="tabbable tabs-left">
            ...
            <div class="ava-conteudo"> 
              ...
              <div class="tab-pane active" id="tab_editar">
                <h2 class="tituloOpcao"><?= $idioma['titulo_funcao']; ?></h2>
                <? if($_POST["msg"]) { ?>
                ...
                <form method="post" onsubmit="return validateFields(this, regras)" enctype="multipart/form-data" class="form-horizontal">
                  <input name="acao" type="hidden" value="salvar_linkacao" />
                  <? if($url[6] == "editar") {
                    ...
                    $linhaObj->GerarFormulario("formulario_linksacoes",$linha,$idioma);				
                  } else {
                    $linhaObj->GerarFormulario("formulario_linksacoes",$_POST,$idioma);
                  }
                  ?>
                ...
                <table class="table table-bordered table-condensed tabelaSemTamanho" cellspacing="0" cellpadding="5">
                    <tbody><tr>
                        <td bgcolor="#F4F4F4"><strong><?= $idioma['tab_nome']; ?></strong></td>
                        <td bgcolor="#F4F4F4"><strong><?= $idioma['tab_tipo']; ?></strong></td>
                        <td bgcolor="#F4F4F4"><strong><?= $idioma['tab_variavel']; ?></strong></td>
                        <td bgcolor="#F4F4F4"><strong><?= $idioma['tab_url']; ?></strong></td>
                        <td bgcolor="#F4F4F4"><strong><?= $idioma['tab_opcoes']; ?></strong></td>
                    </tr>
                    <?php 
                      foreach ($arrayLinksAcoes as $key => $value) {
                    ?>
                      <tr>
                        <td><?= $value['nome']; ?></td>
                        <td><?= $linksacoes['pt_br'][$value['tipo']]; ?></td>
                        <td style="color: red"><?= $value['variavel']; ?></td>
                        <td><a href="<?= $value['url']; ?>"><?= $value['url']; ?></a></td>
                        <td><a class="btn btn-mini" href="javascript:void(0)" onclick="remover_opcao(<?= $value['idlinkacao']; ?>);">Remover</a></td>
                      </tr>
                    <?php 
                      }
                    ?>
    ...
    <form method="post" id="form_remover_opcao" action="" style="padding-top:15px;">
        <input name="acao" type="hidden" value="remover_acaolink" />
    ...
    <script type="text/javascript">
      function remover_opcao(idOpcao) {
        var msg = "Deseja remover esse link / ação?";
        var confirma = confirm(msg);
        if (confirma) {
            document.getElementById('remover').value = idOpcao;
            document.getElementById('form_remover_opcao').submit();
            return true;
        } else {
            return false;
        }
      ...
      <?php
      foreach($config["formulario_linksacoes"] as $fieldsetid => $fieldset) {
      ...
      jQuery(document).ready(function($) {
          ...
          $("select[name='tipo']").change(function() {
            if($(this).val() == 'L'){
              ...
              regras.push("required,form_url,Informe a URL do link.");
            }
            if($(this).val() == 'A'){
              $("input[name='url']").parent().parent().hide();
              $("input[name='url']").val('');
              regras.splice("required,form_url,Informe a URL do link.");
            }
            if($(this).val() == ''){
              $("input[name='url']").parent().parent().hide();
              $("input[name='url']").val('');
