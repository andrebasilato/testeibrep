  ...
  <div class="container-fluid">
    <section id="global">
      ...
      <ul class="breadcrumb">
        ...
        <li><a href="/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/<?= $linha["idava"]; ?>/avaliacoes"><?= $idioma["pagina_titulo_interno"]; ?></a> <span class="divider">/</span></li>
        <? if($url[5] == "cadastrar") { ?>
          ...
          <div class="tabbable tabs-left">
            ...
            <div class="ava-conteudo"> 
              ...
              <?php if($url[5] != "cadastrar") { ?><h2 class="tituloEdicao">
				<?php echo $linha["nome"]; ?></h2>
                <?php include("inc_submenu_avaliacoes.php"); ?>
              <?php } ?>
              <div class="tab-pane active" id="tab_editar">
                ...
                <form method="post" onsubmit="return validateFields(this, regras)" enctype="multipart/form-data" class="form-horizontal">
                  <input name="acao" type="hidden" value="salvar_avaliacao" />
                  <? if($url[6] == "editar") {
                    echo '<input type="hidden" name="'.$config["banco_avaliacoes"]["primaria"].'" id="'.$config["banco_avaliacoes"]["primaria"].'" value="'.$linha[$config["banco_avaliacoes"]["primaria"]].'" />';
                    foreach($config["banco_avaliacoes"]["campos_unicos"] as $campoid => $campo) {
                    ?>
                      <input name="<?= $campo["campo_form"]; ?>_antigo" id="<?= $campo["campo_form"]; ?>_antigo" type="hidden" value="<?= $linha[$campo["campo_banco"]]; ?>" />
                    <? 
                    }					  
                    $linhaObj->GerarFormulario("formulario_avaliacoes",$linha,$idioma);				
                  } else {
                    $linhaObj->GerarFormulario("formulario_avaliacoes",$_POST,$idioma);
                  }
                  ?>
    ...
    <script type="text/javascript">
      ...
      <?php
      foreach($config["formulario_avaliacoes"] as $fieldsetid => $fieldset) {
        ...
        <? foreach($config["formulario_avaliacoes"] as $fieldsetid => $fieldset) {
          foreach($fieldset["campos"] as $campoid => $campo) {
            ...
            if($campo["mascara"]){ ?>
				...
				<?php } elseif ($campo["mascara"] == "999:99:99?9") { ?> 
          $('#<?= $campo["id"]; ?>').focusout(function(){
            var intervalo, element;
            element = $(this);
            element.unmask();
            intervalo = element.val().replace(/\D/g, '');
            if(intervalo.length > 6) {
              element.mask("999:99:99?9");
            } else {
              element.mask("99:99:99?9");
            }
          }).trigger('focusout');
	...
	// Bloqueia o campo "Disciplina da nota" caso exista avaliações realizadas
    if(<? echo $qtdeTentativas ?> > 0) {
       $("#iddisciplina_nota").attr('readonly', true);
       $("#iddisciplina_nota").css('pointer-events','none');
       $("#iddisciplina_nota").css('touch-action','none');
       $("#iddisciplina_nota").attr('tabindex','-1');
       $("#iddisciplina_nota").css('aria-disabled','true');
        }
