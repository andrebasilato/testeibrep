        ...
        <div class="row-fluid">
          <div class="span12">
            <div class="forum-form marge30-up">
              <div class="forum-base-titulo corbgpadrao"><?php if($url[10] != "moderar") { echo $idioma["responder"]; } else { echo $idioma["moderar"]; } ?></div>
              <div>
                <form method="post" onsubmit="return validateFields(this, regras)" enctype="multipart/form-data">
                  <input name="acao" id="acao" type="hidden" value="<?php if($url[10] != "moderar") { ?>responder_topico<?php } else { ?>moderar_mensagem<?php } ?>" />
                  <?php if($url[9] && $url[10] == "responder") { ?>
                    <input name="idmensagem_associada" id="idmensagem_associada" type="hidden" value="<?php echo $url[9]; ?>" />
                  <?php } elseif($url[9] && $url[10] == "moderar") { ?>
                    <input name="idmensagem" id="idmensagem" type="hidden" value="<?php echo $url[9]; ?>" />
                  <?php } ?>
                  <label><?php echo $idioma["mensagem"]; ?></label>
                  <textarea id="mensagem" name="<?php if($url[10] != "moderar") { ?>mensagem<?php } else { ?>moderar<?php } ?>"><?php if($mensagem["moderado"] == "S") { echo nl2br($mensagem["moderado_mensagem"]); } else { echo nl2br($mensagem["mensagem"]); } ?></textarea>
                  <?php if($url[10] != "moderar") { ?>
