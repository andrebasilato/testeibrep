            ...
            <div class="ava-conteudo"> 
              ...
              <div class="tab-pane active" id="tab_editar">
                ...
                <form class="well" method="post" id="form">
                  ...
                  <?php if($perfil["permissoes"][$url[2]."|32"]) { ?>    
                  <select id="perguntas" name="perguntas"></select>
                  <br />
                  <br />
                  <input type="hidden" id="acao" name="acao" value="associar_perguntas">
              ...
              <table class="table table-striped tabelaSemTamanho">
                  ...
                  <tbody>
                      <?php if(count($perguntas) > 0) { ?>
                          <?php foreach($perguntas as $associacao) { ?>
                              <tr>                                                
                                  <td><?php echo $associacao["nome"]; ?></td>
                                  <td>
                                    <?php if($perfil["permissoes"][$url[2]."|32"]) { ?>
                                    <a href="javascript:void(0);" class="btn btn-mini" data-original-title="<?= $idioma["btn_remover"]; ?>" data-placement="left" rel="tooltip" onclick="remover(<?php echo $associacao["idsimulado_disciplina_pergunta"]; ?>)"><i class="icon-remove"></i></a>
                                    <?php } else { ?>
    ...
    <script type="text/javascript">
		$(document).ready(function(){                
			$("#perguntas").fcbkcomplete({
				json_url: "/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/<?= $url[3]; ?>/<?= $url[4]; ?>/<?= $url[5]; ?>/json/associar_pergunta",
				addontab: true,
				height: 10,
				maxshownitems: 10,
				cache: true,
