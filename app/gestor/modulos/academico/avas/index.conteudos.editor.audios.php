...
$config["listagem_audios"] = array(
  ...
  array(
	...
	"tipo" => "php", 
	"valor" => '
	if(!empty($linha["imagem_exibicao_servidor"])) {
		$imgSrc = "/avas_audios_imagem_exibicao/".$linha["imagem_exibicao_servidor"];
	}
	return "<div class=\"audio\">
        <button type=\"button\" class=\"btn btn-mini useAudio\" data-url=\"/storage/avas_audios_arquivo/".$linha["arquivo_servidor"]."\" imagem=\"".$imgSrc."\">Inserir</button>
        <a class=\"btn dropdown-toggle btn-mini\" data-original-title=\"".$idioma["tabela_opcoes_tooltip"]."\" href=\"/".$this->url["0"]."/".$this->url["1"]."/".$this->url["2"]."/".$this->url["3"]."/".$this->url["4"]."/".$linha["idaudio"]."/opcoes\" data-placement=\"left\" rel=\"tooltip facebox\">".$idioma["tabela_opcoes"]."</a>
     </div>"
    ',
	"busca_botao" => true,
	"tamanho" => "120"
...
if(isset($url[5])){			
  if($url[5] == "cadastrar") {
	...
	include("telas/".$config["tela_padrao"]."/formulario.conteudos.editor.audios.php");
	exit();
  } else {	
	...
	if($linha) {
	  switch($url[6]) {
		...
		case "remover":			
		  ...
		  include("telas/".$config["tela_padrao"]."/remover.conteudos.editor.audios.php");
		break;
		case "opcoes":			
  ...
  include("telas/".$config["tela_padrao"]."/index.conteudos.editor.audios.php");
}
?>