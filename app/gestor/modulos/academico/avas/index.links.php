  ...
  array(
	...
	"campos" => array( // Campos do formulario																						
	  ...
	  array(
		...
		"pasta" => "avas_links_imagem_exibicao", 
		"download" => true,
	  ...
	  array(
		"id" => "idava_link", // Id do atributo HTML
...
$config["listagem_links"] = array(
  array(
	"id" => "idlink",
	"variavel_lang" => "tabela_idlink", 
	"tipo" => "banco", 
	"coluna_sql" => "idlink", 
	"valor" => "idlink", 
	"busca" => true,
  ...
  array(
	...
	"tipo" => "php", 
	"valor" => 'return "<a class=\"btn dropdown-toggle btn-mini\" data-original-title=\"".$idioma["tabela_opcoes_tooltip"]."\" href=\"/".$this->url["0"]."/".$this->url["1"]."/".$this->url["2"]."/".$this->url["3"]."/".$this->url["4"]."/".$linha["idlink"]."/opcoes\" data-placement=\"left\" rel=\"tooltip facebox\">".$idioma["tabela_opcoes"]."</a>"',
	"busca_botao" => true,
	"tamanho" => "80"
  ) 
 );
...
$linhaObj->Set("monitora_onde",$config["monitoramento"]["onde_links"]);
$linhaObj->Set("idava",intval($url[3]));

$linhaObj->config["banco"] = $config["banco_links"];
$linhaObj->config["formulario"] = $config["formulario_links"];

if($_POST["acao"] == "salvar_link"){
  ...
  if($_POST[$config["banco_links"]["primaria"]]) 
	$salvar = $linhaObj->ModificarLink();
  else 
	$salvar = $linhaObj->CadastrarLink();
...
} elseif($_POST["acao"] == "remover_link") {
  $linhaObj->verificaPermissao($perfil["permissoes"], $url[2]."|9");
  $linhaObj->Set("post",$_POST);
  $remover = $linhaObj->RemoverLink();
  if($remover["sucesso"]){
...
if(isset($url[5])){			
  if($url[5] == "cadastrar") {
	$linhaObj->verificaPermissao($perfil["permissoes"], $url[2]."|8");
	include("idiomas/".$config["idioma_padrao"]."/formulario.links.php");
	include("telas/".$config["tela_padrao"]."/formulario.links.php");
	exit();
  } else {	
	$linhaObj->Set("id",intval($url[5]));
	$linhaObj->Set("campos","l.*, a.nome as ava");	
	$linha = $linhaObj->RetornarLink();
	...
	if($linha) {
	  switch($url[6]) {
		...
		case "remover":			
		  $linhaObj->verificaPermissao($perfil["permissoes"], $url[2]."|9");
		  include("idiomas/".$config["idioma_padrao"]."/remover.links.php");
		  include("telas/".$config["tela_padrao"]."/remover.links.php");
		break;
		case "opcoes":			
		  include("idiomas/".$config["idioma_padrao"]."/opcoes.links.php");
		  include("telas/".$config["tela_padrao"]."/opcoes.links.php");
		break;
		case "download":
  ...
  if(!$_GET["cmp"]) $_GET["cmp"] = $config["banco_links"]["primaria"];
  $linhaObj->Set("ordem_campo",$_GET["cmp"]);
  $linhaObj->Set("campos","l.*, a.nome as ava");	
  $dadosArray = $linhaObj->ListarTodasLink();		
  include("idiomas/".$config["idioma_padrao"]."/index.links.php");
  include("telas/".$config["tela_padrao"]."/index.links.php");
}
?>