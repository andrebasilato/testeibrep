<?php
$config["formulario_perguntas"] = array(
  array(
	...
	"campos" => array( // Campos do formulario																						
	  ...
	  array(
		...
		"pasta" => "avas_perguntas_imagem_exibicao", 
		"download" => true,
...
$config["listagem_perguntas"] = array(
  array(
	"id" => "idpergunta",
  ...
  array(
	...
	"tipo" => "php", 
	"valor" => 'return "<a class=\"btn dropdown-toggle btn-mini\" data-original-title=\"".$idioma["tabela_opcoes_tooltip"]."\" href=\"/".$this->url["0"]."/".$this->url["1"]."/".$this->url["2"]."/".$this->url["3"]."/".$this->url["4"]."/".$linha["idpergunta"]."/opcoes\" data-placement=\"left\" rel=\"tooltip facebox\">".$idioma["tabela_opcoes"]."</a>"',
	"busca_botao" => true,
	"tamanho" => "80"
  ) 
 );
...
$linhaObj->Set("monitora_onde",$config["monitoramento"]["onde_perguntas"]);
$linhaObj->Set("idava",intval($url[3]));

$linhaObj->config["banco"] = $config["banco_perguntas"];
$linhaObj->config["formulario"] = $config["formulario_perguntas"];

if($_POST["acao"] == "salvar_pergunta"){
  ...
  if($_POST[$config["banco_perguntas"]["primaria"]]) 
	$salvar = $linhaObj->ModificarPergunta();
  else 
	$salvar = $linhaObj->CadastrarPergunta();
...
if(isset($url[5])){			
  if($url[5] == "cadastrar") {
	$linhaObj->verificaPermissao($perfil["permissoes"], $url[2]."|8");
	include("idiomas/".$config["idioma_padrao"]."/formulario.perguntas.php");
	include("telas/".$config["tela_padrao"]."/formulario.perguntas.php");
	exit();
  } else {	
	$linhaObj->Set("id",intval($url[5]));
	$linhaObj->Set("campos","p.*, a.nome as ava");	
	$linha = $linhaObj->RetornarPergunta();
	...
	if($linha) {
	  switch($url[6]) {
		...
		case "remover":			
		  $linhaObj->verificaPermissao($perfil["permissoes"], $url[2]."|9");
		  include("idiomas/".$config["idioma_padrao"]."/remover.perguntas.php");
		  include("telas/".$config["tela_padrao"]."/remover.perguntas.php");
		break;
		case "opcoes":			
		  include("idiomas/".$config["idioma_padrao"]."/opcoes.perguntas.php");
		  include("telas/".$config["tela_padrao"]."/opcoes.perguntas.php");
		break;
		case "download":
  ...
  if(!$_GET["cmp"]) $_GET["cmp"] = $config["banco_perguntas"]["primaria"];
  $linhaObj->Set("ordem_campo",$_GET["cmp"]);
  $linhaObj->Set("campos","p.*, a.nome as ava");	
  $dadosArray = $linhaObj->ListarTodasPergunta();		
  include("idiomas/".$config["idioma_padrao"]."/index.perguntas.php");
  include("telas/".$config["tela_padrao"]."/index.perguntas.php");
}
?>