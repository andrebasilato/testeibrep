  ...
  <div class="container-fluid">
            ...
            <div class="tab-content">
              <div class="tab-pane active" id="tab_editar">        
                ...
                <form class="well" method="post">
                  <p><?= $idioma["form_prova"]; ?></p>
                  <?php if($perfil["permissoes"][$url[2]."|2"]) { ?>    
                    <select id="provas" name="provas"></select>
                    <br />
                    <br />
                    <input type="hidden" id="acao" name="acao" value="associar_prova">
                    <input type="submit" class="btn" value="<?= $idioma["btn_adicionar"]; ?>" />
				  <?php } else { ?>
                    <select id="provas" name="provas" disabled="disabled"></select>
                    <br />
                    <br />
                ...
                <br />
                <form method="post" id="remover_prova" name="remover_prova">
                  <input type="hidden" id="acao" name="acao" value="remover_prova">
                ...
                <table class="table table-striped tabelaSemTamanho">
                  <thead>
                    <tr>
                      ...
                      <th><?= $idioma["listagem_data_realizacao"]; ?></th>
                      <th width="60"><?= $idioma["listagem_opcoes"]; ?></th>
                    </tr>
                  </thead>
                  <tbody>
                      <tr>
                        <?php if ($linha['id_prova_presencial']) { ?>
                            <td><?= $linha['id_prova_presencial']?></td>
                            <td><?php echo formataData($linha['data_agendamento'], 'br', 0); ?></td>
                            <td>
                              <?php if($perfil["permissoes"][$url[2]."|2"]) { ?>
                                <a href="javascript:void(0);" class="btn btn-mini" data-original-title="<?= $idioma["btn_remover"]; ?>" data-placement="left" rel="tooltip" onclick="remover(<?php echo $linha["id_solicitacao_prova"]; ?>)"><i class="icon-remove"></i></a>
                              <?php } else { ?>
    ...
    <script type="text/javascript">
	  $(document).ready(function(){               
		$("#provas").fcbkcomplete({
		  json_url: "/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/<?= $url[3]; ?>/json/associar_prova",
		  addontab: true,
		  height: 10,
		  maxshownitems: 10,
		  cache: true,
	  ...
	  function remover(id) {
		...
		if(confirma) {
		  ...
		  document.getElementById("remover_prova").submit();
		} 
	  }
	</script>
  </div>
