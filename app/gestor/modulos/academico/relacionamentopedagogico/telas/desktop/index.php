  ...
  <div class="row-fluid">
    <div class="span12">
      <div class="box-conteudo">
          ...
          <div class="tabbable tabs-left">
            <div class="tab-content">
              <div class="tab-pane active" id="tab_editar">
                ...
                <? } ?>
                <form class="well" action="/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/administrar/" method="post" id="form_pessoa" name="form_pessoa" onsubmit="return Seturl();">
                  ...
                  <? /*<a class="btn btn-mini" href="#ficha_financeira" rel="facebox" onclick="seleciona_pessoa()"><?= $idioma["btn_abrir_ficha"]; ?></a> */?>
                  <?php } else { ?>
                  <select id="pessoas" name="pessoas" disabled="disabled"></select>
                  <br />
                  <br />
                  <a href="javascript:void(0);" rel="tooltip" data-original-title="<?= $idioma["btn_permissao_inserir"]; ?>" data-placement="right" class="btn disabled"><?= $idioma["btn_abrir_ficha"]; ?></a>
				  <?php } ?>
                  <br /><br />
                </form>
          ...
          <?php  $linhaObj->GerarTabela($mensagensArray,$_GET["q"],$idioma,'listagem_mensagens','tabelaSemTamanho'); ?>
      ...
      <script>
  				function Seturl() {
            pessoa =  document.getElementById('pessoas').selectedIndex;
            if (pessoa==-1){
              alert("Selecione a pessoa que deseja abrir a ficha do relacionamento comercial.");
              return false;
            }
  					document.form_pessoa.action = "/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/administrar/" + document.getElementById('pessoas').options[document.getElementById('pessoas').selectedIndex].value;
  				  return true;
          }
				</script>
              </div>
    ...
    <script type="text/javascript">
		$(document).ready(function(){                
			$("#pessoas").fcbkcomplete({
				json_url: "/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/json/pesquisar_pessoas",
				addontab: true,
				height: 10,
				maxshownitems: 15,
				cache: true,
