...
$config["listagem"] = array(
  array(
	"id" => "idarea",
	"variavel_lang" => "tabela_idarea", 
	"tipo" => "php", 
	"coluna_sql" => "idarea", 
	"valor" => '
			$diferenca = dataDiferenca($linha["data_cad"], date("Y-m-d H:i:s"), "H");
			if($diferenca > 24) {
				return "<span title=\"$diferenca\">".$linha["idarea"]."</span>";
			} else {
				return "<span title=\"$diferenca\">".$linha["idarea"]."</span> <i class=\"novo\"></i>";
			}
			', 
	"busca" => true,
  ...
  array(
	...
	"tipo" => "php", 
	"valor" => 'return "<a class=\"btn dropdown-toggle btn-mini\" data-original-title=\"".$idioma["tabela_opcoes_tooltip"]."\" href=\"/".$this->url["0"]."/".$this->url["1"]."/".$this->url["2"]."/".$linha["idarea"]."/opcoes\" data-placement=\"left\" rel=\"tooltip facebox\">".$idioma["tabela_opcoes"]."</a>"',
	"busca_botao" => true,
	"tamanho" => "80"
  ) 			
);						   
