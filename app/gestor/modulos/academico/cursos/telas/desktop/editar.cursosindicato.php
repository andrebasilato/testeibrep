...
<body>
  <div class="span10">
    <section id="formulario_cliente">  
      ...
      <? } ?>
      <form method="post" action="/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/<?= $url[3]; ?>/<?= $url[4]; ?>" onsubmit="return validateFields(this, regras)" enctype="multipart/form-data" class="form-horizontal" target="_parent">
        <?
        if($curso_sindicato) {
          echo '<input type="hidden" name="idcurso_sindicato" id="idcurso_sindicato" value="'.$curso_sindicato["idcurso_sindicato"].'" />';
          foreach($config["banco_cursosindicato"]["campos_unicos"] as $campoid => $campo) {
          ?>
            <input name="<?= $campo["campo_form"]; ?>_antigo" id="<?= $campo["campo_form"]; ?>_antigo" type="hidden" value="<?= $curso_sindicato[$campo["campo_banco"]]; ?>" />
          <? 
          }
          $linhaObj->GerarFormulario("formulario_cursosindicato",$curso_sindicato,$idioma);
        }
        ?>            
        <div class="form-actions">
          <input name="acao" type="hidden" value="editar_dados_cursosindicato" />
...
<script type="text/javascript" src="/assets/js/jquery.limit-1.2.source.js"></script>
    ...
    <?php
    foreach($config["formulario_cursosindicato"] as $fieldsetid => $fieldset) {
...
?>
    ...
    jQuery(document).ready(function($) {
        ...
        $('#sms_boas_vindas_sindicato').limit('160','#charsLeft');
        if($('#sms_boas_vindas_sindicato').val().length == 160){
          $('#charsLeft').html(0);
        }
