  ...
  <div class="container-fluid">
              ...
              <div class="tab-pane active" id="tab_editar">
                ...
                <form class="well" method="post" id="form">
                  ...
                  <?php if($perfil["permissoes"][$url[2]."|5"]) { ?>    
                    <select id="areas" name="areas"></select>
                    <br />
                    <br />
                    <input type="hidden" id="acao" name="acao" value="associar_area">
                    <input type="submit" class="btn" value="<?= $idioma["btn_adicionar"]; ?>" />
				  <?php } else { ?>
                    <select id="areas" name="areas" disabled="disabled"></select>
                    <br />
                    <br />
                ...
                <br />
                <form method="post" id="remover_area" name="remover_area">
                  <input type="hidden" id="acao" name="acao" value="remover_area">
                  <input type="hidden" id="remover" name="remover" value="">
                </form>
                <table class="table table-striped tabelaSemTamanho">
                  <thead>
                    <tr>
                      <th><?= $idioma["listagem_area"]; ?></th>
					  ...
					  <?php foreach($associacoesArray as $ind => $associacao) { ?>
                        <tr>                                                
                          ...
                          <td>
                            ...
                            <a href="javascript:void(0);" class="btn btn-mini" data-original-title="<?= $idioma["btn_remover"]; ?>" data-placement="left" rel="tooltip" onclick="remover(<?php echo $associacao["idcurso_area"]; ?>)"><i class="icon-remove"></i></a>
                            <?php } else { ?>
    ...
    <script type="text/javascript">
	  $(document).ready(function(){                
		$("#areas").fcbkcomplete({
		  json_url: "/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/<?= $url[3]; ?>/json/associar_area",
		  addontab: true,
		  height: 10,
		  maxshownitems: 10,
		  cache: true,
	  ...
	  function remover(id) {
		...
		if(confirma) {
		  ...
		  document.getElementById("remover_area").submit();
		} 
	  }
	</script>
  </div>
