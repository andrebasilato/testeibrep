...
<div class="container-fluid">
              		...
              		<div class="tab-content">
                		<div class="tab-pane active" id="tab_editar">
    						...
    						<form class="well" method="post">
                              <p><?= $idioma["form_contrato"]; ?></p>
                              <?php if($perfil["permissoes"][$url[2]."|5"]) { ?>    
                                <select id="contratos" name="contratos"></select>
                                <br />
                                <br />
                                <input type="hidden" id="acao" name="acao" value="associar_contrato">
                                <input type="submit" class="btn" value="<?= $idioma["btn_adicionar"]; ?>" />
							  <?php } else { ?>
                                <select id="contratos" name="contratos" disabled="disabled"></select>
                                <br />
                                <br />
                            ...
                            <br />
     						<form method="post" id="remover_contrato" name="remover_contrato">
                              <input type="hidden" id="acao" name="acao" value="remover_contrato">
                            ...
                            <table class="table table-striped">
        						...
        						<tbody>
                                    <?php if(count($contratosArray) > 0) { ?>
										<?php foreach($contratosArray as $ind => $contrato) { ?>
                                            <tr>
                                                <td><?php echo $contrato["idcontrato"]; ?></td>
                                                <td><?php echo $contrato["nome"]; ?> (<?php echo $contrato["tipo"]; ?>)</td>
                                                <td>
                                                  <a class="btn btn-mini" rel="tooltip facebox" data-placement="left" data-original-title="<?=$idioma["visuliza_contrato"];?>" href="/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/<?= $url[3]; ?>/visualizarcontrato/<?= $contrato["idcontrato"]; ?>"><i class="icon-eye-open"></i></a>
												  <?php if($perfil["permissoes"][$url[2]."|6"]) { ?>
                                                    <a href="javascript:void(0);" class="btn btn-mini" data-original-title="<?= $idioma["btn_remover"]; ?>" data-placement="left" rel="tooltip" onclick="remover(<?php echo $contrato["idgrupo_contrato"]; ?>)"><i class="icon-remove"></i></a>
                                                  <?php } else { ?>
									...
									<?php } else { ?>
                                   <!--     <tr>
                                            <td colspan="3"><?= $idioma["form_contrato"]; ?></td>
                                        </tr> -->
									<?php } ?>
                                </tbody>
                            </table>
    ...
    <script type="text/javascript">
		$(document).ready(function(){                
			$("#contratos").fcbkcomplete({
				json_url: "/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/<?= $url[3]; ?>/associar_contratos",
				addontab: true,
				height: 10,
				maxshownitems: 10,
				cache: true,
		...
		function remover(id) {
			...
			if(confirma) {
				...
				document.getElementById("remover_contrato").submit();
			} 
		}
	</script>
</div>
