                ...
                <tbody>
				  <?php if(count($cursos) > 0) { ?>
					<?php foreach($cursos as $ind => $curso) { ?>
                      <tr>
                        <td><?php echo $curso["idcurso"]; ?> - <?php echo $curso["nome"]; ?></td>
                        <td>
						  <?php if($perfil["permissoes"][$url[2]."|7"]) { ?>
                          <a href="javascript:void(0);" class="btn btn-mini" data-original-title="<?= $idioma["btn_remover"]; ?>" data-placement="left" rel="tooltip" onclick="remover(<?php echo $curso["idcontrato_curso"]; ?>)"><i class="icon-remove"></i></a>
                          <?php } else { ?>
