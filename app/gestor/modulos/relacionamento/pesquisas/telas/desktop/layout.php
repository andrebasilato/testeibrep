...
<div class="container-fluid">
    						...
    						<form class="well" method="post">
                                ...
                                <div id="div_form_botao_variaveis" style="display: none;">
                                    ...
                                    <table class="table-striped table-bordered table-condensed" width="100%">
                                        <tr>
                                            <td colspan='4' style='border:0px; border-bottom: 1px solid #DDDDDD; background-color:#F8F8F8' width='100%'><strong><?= $idioma["tabela_variaveis"]; ?></strong></td>
                                        </tr>
                                    <? if($associacoesArray){?>
                                    <? foreach($associacoesArray as $ind => $val){ ?>
                                        <tr>
                                            <td><strong>[[P][<?= $val["idpergunta"]; ?>]]</strong></td>
                                            <td><?= $val["nome"]; ?></td>
                                        </tr>
                                    <? } ?>
                                    <? }else{ ?>
                                        <tr>
                                            <td colspan="2"><?= $idioma["tabela_perguntas_vazio"] ?></td>
                                        </tr>
                                    <? } ?>
                                    </table>
                                </div>
                                <div id="div_form_botao_variaveis_imagens" style="display: none;">
                                    ...
                                    <? foreach($associacoesImagensArray as $ind => $val){ ?>
                                        <tr>
                                            <td><strong>[[I][<?= $val["idpesquisa_imagem"]; ?>]]</strong></td>
                                            <td><?= $val["nome"]; ?></td>
                                            <td><a href="<?php echo "/".$url[0]."/".$url[1]."/".$url[2]."/".$url[3]."/visualiza_imagem/".$val["idpesquisa_imagem"]; ?>" rel="facebox tooltip" class="btn btn-mini" data-original-title="<?= $idioma["btn_visualizar"]; ?>" data-placement="left"><i class="icon-picture"></i> <?php echo $idioma["visualizar"]; ?></a></td>
                                        </tr>
                                    <? } ?>
                                    <? }else{ ?>
                                        <tr>
                                ...
                                <textarea id="layout" name="layout"><?= $linha["layout"]; ?></textarea>
                                <br />
                                <br />
    							<input type="hidden" id="acao" name="acao" value="salvar_layout">
                                <input type="submit" class="btn btn-primary" value="<?= $idioma["btn_adicionar"]; ?>" />
								<?php } else { ?>
                                <textarea name="layout" disabled="disabled" style="width:100%"></textarea>
                                <br />
                                <br />
    ...
    <script type="text/javascript">
		...
		var editor = CKEDITOR.replace( 'layout', {
/*
