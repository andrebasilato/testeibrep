...
include("../classes/emailsautomaticos.class.php");
	include("config.php");
	...
	$linhaObj = new Emails_Automaticos();
	...
	if($_POST["acao"] == "salvar"){
		...
		if($salvar["sucesso"]){
			...
			$linhaObj->Set("url","/".$url[0]."/".$url[1]."/".$url[2]."/".$url[3]."/email_ofertas");
			$linhaObj->Processando();
		}
	...
	elseif($_POST["acao"] == "associar_sindicato"){
		...
		if($salvar["sucesso"]){
			...
			$linhaObj->Set("url","/".$url[0]."/".$url[1]."/".$url[2]."/".$url[3]."/email_sindicatos");
			$linhaObj->Processando();
		}
			...
			if($linha) {
				...
				switch ($url[4]) {
					...
					case "email_ofertas":			
						...
						$linhaObj->Set("limite",-1);
						$linhaObj->Set("ordem_campo","nome");
						$linhaObj->Set("campos","eo.idemail_oferta, eo.idemail, o.idoferta, o.nome");
						$associacoesArray = $linhaObj->ListarOfertasAss();
						
						include("idiomas/".$config["idioma_padrao"]."/email.ofertas.php");
						include("telas/".$config["tela_padrao"]."/email.ofertas.php");
						break;
					case "email_sindicatos":			
						...
						$linhaObj->Set("limite",-1);
						$linhaObj->Set("ordem_campo","nome");
						$linhaObj->Set("campos","ei.idemail_sindicato, ei.idemail, i.idsindicato, i.nome_abreviado");
						$associacoesArray = $linhaObj->ListarSindicatosAss();
						
						include("idiomas/".$config["idioma_padrao"]."/email.sindicatos.php");
						include("telas/".$config["tela_padrao"]."/email.sindicatos.php");
						break;
					case "download":
