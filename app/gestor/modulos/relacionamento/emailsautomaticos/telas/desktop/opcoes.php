      ...
      <ul class="nav nav-tabs nav-stacked">
          ...
          <li>
              <? if($linhaObj->verificaPermissao($perfil["permissoes"], $url[2]."|10", NULL)){ ?>
                  <a href="/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/<?php echo $linha["idemail"]; ?>/email_sindicatos"> <i class="icon-edit"></i> <? echo $idioma["email_sindicatos"]; ?></a>
              <? } else { ?>
                  <a href="javascript:void(0)" data-original-title="<?= $idioma["opcao_permissao"] ?>"  data-placement="left" rel="tooltip" style="color:#999;"> <i class="icon-edit icon-white"></i> <? echo $idioma["email_sindicatos"]; ?></a>
              <?php } ?>        
          </li>
