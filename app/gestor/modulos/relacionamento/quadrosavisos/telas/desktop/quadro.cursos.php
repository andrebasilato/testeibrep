										...
										<tbody>
											<?php if(count($associacoesArray) > 0) { ?>
													<?php foreach($associacoesArray as $ind => $associacao) { ?>
															<tr>
																	<td><?php echo $associacao["idquadro_curso"]; ?></td>
																	<td><?php echo $associacao["nome"]; ?></td>
																	<td>
																		<?php if($perfil["permissoes"][$url[2]."|9"]) { ?>
																		<a href="javascript:void(0);"  class="btn btn-mini" data-original-title="<?= $idioma["btn_remover"]; ?>" data-placement="left" rel="tooltip" onclick="remover(<?php echo $associacao["idquadro_curso"]; ?>)"><i class="icon-remove"></i></a>
																		<?php } else { ?>
