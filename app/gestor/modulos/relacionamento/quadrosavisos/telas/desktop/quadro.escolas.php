        						...
        						<tbody>
                                    <?php if(count($associacoesArray) > 0) { ?>
										<?php foreach($associacoesArray as $ind => $associacao) { ?>
                                            <tr>
                                                <td><?php echo $associacao["idquadro_escola"]; ?></td>
                                                <td><?php echo $associacao["nome_fantasia"]; ?></td>
                                                <td>
                                                  <?php if($perfil["permissoes"][$url[2]."|7"]) { ?>
                                                  <a href="javascript:void(0);"  class="btn btn-mini" data-original-title="<?= $idioma["btn_remover"]; ?>" data-placement="left" rel="tooltip" onclick="remover(<?php echo $associacao["idquadro_escola"]; ?>)"><i class="icon-remove"></i></a>
                                                  <?php } else { ?>
    ...
    <script type="text/javascript">
		$(document).ready(function(){                
			$("#escolas").fcbkcomplete({
				json_url: "/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/<?= $url[3]; ?>/json/associar_escolas",
				addontab: true,
				height: 10,
				maxshownitems: 10,
				cache: true,
