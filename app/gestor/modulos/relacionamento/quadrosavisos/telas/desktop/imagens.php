...
function novo_arquivo(){
	...
	regras.push("formato_arquivo,"+id+",jpg|jpeg|gif|png|bmp,'',<?php echo $idioma['arquivo_invalido']; ?>");
}
</script>
</head>
<body>
  	...
  	<div class="span12">
        <div class="box-conteudo">
                    ...
                    <form method="post" action="" class="well form-horizontal form-inline" enctype="multipart/form-data" onsubmit="return validateFields(this, regras)">
                      ...
                      <?php /*?><button type="button" rel="tooltip" data-original-title="<?= $idioma["salvar_permissao"]; ?>" data-placement="right" class="btn disabled"><?= $idioma["salvar"]; ?></button><?php */?>
                        ...
                        <tbody>
                            <?php if(count($imagensArray) > 0) { ?>
                                <?php foreach($imagensArray as $ind => $imagem) { ?>
                                    <tr>
                                        <td><?php echo $imagem["idquadro_imagem"]; ?></td>
                                        <td><?= $imagem["nome"]; ?></td>
                                        <td><?php echo tamanhoArquivo($imagem["tamanho"]); ?></td>
                                        <td style="text-align:right">
                                          <?php if (strpos($imagem['tipo'],'image') !== false and $perfil["permissoes"][$url[2]."|13"]) { ?>
                                          <a href="<?php echo "/".$url[0]."/".$url[1]."/".$url[2]."/".$url[3]."/visualiza_imagem/".$imagem["idquadro_imagem"]; ?>" rel="facebox tooltip" class="btn btn-mini" data-original-title="<?= $idioma["btn_visualizar"]; ?>" data-placement="left"><i class="icon-picture"></i> <?php echo $idioma["visualizar"]; ?></a>
                                           <?php } ?>                                          
                                           <?php if($perfil["permissoes"][$url[2]."|12"]) { ?>
                                          <a href="<?php echo "/".$url[0]."/".$url[1]."/".$url[2]."/".$url[3]."/download/".$imagem["idquadro_imagem"]; ?>" class="btn btn-mini" data-original-title="<?= $idioma["btn_download"]; ?>" data-placement="left" rel="tooltip">
                                          <i class="icon-download-alt"></i> <?php echo $idioma["baixar"]; ?>
                                          </a>
                                          <? }else{ ?>
                                          <?php /*?><button class="btn btn-mini disabled" data-original-title="<?= $idioma["btn_remover_permissao_download"]; ?>" data-placement="left" rel="tooltip"><i class="icon-download-alt"></i> <?php echo $idioma["baixar"]; ?></button><?php */?>
                                          ...
                                          <a href="javascript:void(0);" class="btn btn-mini" data-original-title="<?= $idioma["btn_remover"]; ?>" data-placement="left" rel="tooltip" onclick="remover(<?php echo $imagem["idquadro_imagem"]; ?>)"><i class="icon-remove"></i></a>                                          
                                          <?php } else { ?>
                                          <?php /*?><button class="btn btn-mini disabled" data-original-title="<?= $idioma["btn_remover_permissao_excluir"]; ?>" data-placement="left" rel="tooltip"><i class="icon-remove"></i></button><?php */?>
...
<script type="text/javascript">
	...
	regras.push("formato_arquivo,arquivos[1],jpg|jpeg|gif|png|bmp,'',<?php echo $idioma['arquivo_invalido']; ?>");	
</script>
<script type="text/javascript">		
	function remover(id) {
