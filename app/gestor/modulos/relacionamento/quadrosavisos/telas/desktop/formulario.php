        ...
        <div class="box-conteudo">
            ...
            <? if($_POST["msg"]) { ?>
                ...
                <? if(count($salvar["erros"]) > 0){ ?>
                    <div class="alert alert-error fade in" style="width:772px">
                        <a href="javascript:void(0);" class="close" data-dismiss="alert">×</a>
            ...
            <div class="tabbable tabs-left">
            	<? incluirTela("inc_menu_edicao",$config,$usuario); ?>
                ...
                <form method="post" onsubmit="return valida_form(this, regras)" enctype="multipart/form-data" class="form-horizontal">
                    ...
                    <? if($url[4] == "editar") {
                        ...
                        //$linha["associacoesImagensArray"] = $associacoesImagensArray;
					...
					//	unset($config["formulario"][0]["campos"][2]);
						unset($config["formulario"][0]["campos"][3]);
	...
	jQuery(document).ready(function($) {
		$("#data_de").datepicker({					 
			...
			onSelect: function(theDate){				
				$("#data_ate").datepicker("option", "minDate", theDate)
				$("#data_ate").datepicker({
					currentText: 'Now', 
					dateFormat: 'dd/mm/yy'
				})
				$("#data_de").datepicker("option", "minDate", theDate)				
			}
		})		
	<?
		foreach($config["formulario"] as $fieldsetid => $fieldset) {
...
return element;
	...
	<?
	  ...
	  if($campo["contador"]){
	  ?>
	  $('#<?= $campo["id"]; ?>').limit('<?=$campo["contador"];?>','#<?= $campo["id"]; ?>_<?=$campo["contador"];?>');
	  <?
	  }
				if($campo["botao_hide"]){
				?>
