                        ...
                        <tbody>
                            <?php if(count($imagensArray) > 0) { ?>
                                <?php foreach($imagensArray as $ind => $imagem) { ?>
                                    <tr>
                                        <td><?php echo $imagem["idemail_imagem"]; ?></td>
                                        <td><?= $imagem["nome"]; ?></td>
                                        <td><?php echo tamanhoArquivo($imagem["tamanho"]); ?></td>
                                        <td style="text-align:right">
                                          <?php if (strpos($imagem['tipo'],'image') !== false) { ?> <a href="<?php echo "/".$url[0]."/".$url[1]."/".$url[2]."/".$url[3]."/visualiza_imagem/".$imagem["idemail_imagem"]; ?>" rel="facebox tooltip" class="btn btn-mini" data-original-title="<?= $idioma["btn_visualizar"]; ?>" data-placement="left"><i class="icon-picture"></i> <?php echo $idioma["visualizar"]; ?></a> <?php } ?>                                          
                                          <?php /*<a href="<?php echo "/".$url[0]."/".$url[1]."/".$url[2]."/".$url[3]."/download/".$imagem["idemail_imagem"]; ?>">
										  ...
										  <a class="btn btn-mini" data-original-title="<?= $idioma["btn_download"]; ?>" data-placement="left" rel="tooltip" href="<?php echo "/".$url[0]."/".$url[1]."/".$url[2]."/".$url[3]."/download/".$imagem["idemail_imagem"]; ?>">
                                          <i class="icon-download-alt"></i> <?php echo $idioma["baixar"]; ?>
                                          </a> 
                                          <?php if($perfil["permissoes"][$url[2]."|14"]) { ?>
                                          <a href="javascript:void(0);" class="btn btn-mini" data-original-title="<?= $idioma["btn_remover"]; ?>" data-placement="left" rel="tooltip" onclick="remover(<?php echo $imagem["idemail_imagem"]; ?>)"><i class="icon-remove"></i> </a>                                          
                                          <?php } else { ?>
