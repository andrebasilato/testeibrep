<section id="global">
    ...
    <ul class="breadcrumb">
        <li><? echo $idioma["subcategoria_selecionada"]; ?></li>
    ...
    <ul class="nav nav-tabs nav-stacked">
        ...
        <li>
            <? if ($linhaObj->verificaPermissao($perfil["permissoes"], $url[2] . "|4", NULL)) { ?>
                <a href="/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/<?php echo $linha["idsubcategoria"]; ?>/associar_subcategoria">
                    <i class="icon-edit"></i> <? echo $idioma["associar"]; ?></a>
            <? } else { ?>
                <a href="javascript:void(0)" data-original-title="<?= $idioma["opcao_permissao"] ?>"
                   data-placement="left" rel="tooltip" style="color:#999;"> <i
                        class="icon-edit icon-white"></i> <? echo $idioma["associar"]; ?></a>
            <?php } ?>
        </li>
        ...
        <li>
            <? if ($linhaObj->verificaPermissao($perfil["permissoes"], $url[2] . "|3", NULL)) { ?>
                <a href="/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/<?php echo $linha["idsubcategoria"]; ?>/removersubcategoria">
