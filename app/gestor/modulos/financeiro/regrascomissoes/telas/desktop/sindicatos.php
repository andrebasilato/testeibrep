                            ...
                            <form class="well form-inline" method="post">
                                <?php if ($linhaObj->verificaPermissao($perfil["permissoes"], $url[2] . "|7", NULL)) { ?>
                                    <label class="checkbox">
                                        <input type="checkbox" value="S" id="todas_sindicatos"
                                               name="todas_sindicatos"
                                               <?php if ($linha["todas_sindicatos"] == "S") { ?>checked="checked"<?php } ?>> <?= $idioma["form_todas"]; ?>
                                    </label>
                                    <p class="help-block"><?= $idioma["form_todas_ajuda"]; ?></p>
                                    <br/>
                                    <input type="hidden" id="acao" name="acao" value="salvar">
                                    <input type="hidden" id="acao_todos" name="acao_todos" value="sindicato">
                                    <input type="hidden" name="idregra" name="idregra" value="<?= $url["3"]; ?>"/>
                                    <input type="submit" class="btn" value="<?= $idioma["btn_salvar"]; ?>"/>
                                <?php } else { ?>
                                    <label class="checkbox">
                                        <input type="checkbox" value="S" id="todas_sindicatos_obrigatorio"
                                               name="todas_sindicatos_obrigatorio"
                                               <?php if ($linha["todas_sindicatos_obrigatorio"] == "S") { ?>checked="checked"<?php } ?>
                                            ...
                                            <td>
                                                <?php if ($linhaObj->verificaPermissao($perfil["permissoes"], $url[2] . "|8", NULL)) { ?>
                                                    <a href="javascript:void(0);" class="btn btn-mini"
                                                       ...
                                                       onclick="remover(<?php echo $sindicato["idregra_sindicato"]; ?>)"><i
