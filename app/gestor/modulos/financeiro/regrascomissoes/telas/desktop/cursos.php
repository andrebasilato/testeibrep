...
<div class="container-fluid">
                            ...
                            <form class="well form-inline" method="post">
                                <?php if ($linhaObj->verificaPermissao($perfil["permissoes"], $url[2] . "|11", NULL)) { ?>
                                    <label class="checkbox">
                                        <input type="checkbox" value="S" id="todos_cursos" name="todos_cursos"
                                               <?php if ($linha["todos_cursos"] == "S") { ?>checked="checked"<?php } ?>> <?= $idioma["form_todos"]; ?>
                                    </label>
                                    <p class="help-block"><?= $idioma["form_todos_ajuda"]; ?></p>
                                    <br/>
                                    <input type="hidden" id="acao" name="acao" value="salvar">
                                    <input type="hidden" id="acao_todos" name="acao_todos" value="curso">
                                    <input type="hidden" name="idregra" name="idregra" value="<?= $url["3"]; ?>"/>
                                    <input type="submit" class="btn" value="<?= $idioma["btn_salvar"]; ?>"/>
                                <?php } else { ?>
                                    <label class="checkbox">
                                        <input type="checkbox" value="S" id="todos_cursos_obrigatorio"
                                               name="todos_cursos_obrigatorio"
                                               <?php if ($linha["todos_cursos_obrigatorio"] == "S") { ?>checked="checked"<?php } ?>
                                               disabled="disabled"> <?= $idioma["form_todos"]; ?>
                                    </label>
                                    <p class="help-block"><?= $idioma["form_todos_ajuda"]; ?></p>
                                    <br/>
                                            ...
                                            <td>
                                                <?php if ($linhaObj->verificaPermissao($perfil["permissoes"], $url[2] . "|12", NULL)) { ?>
                                                    <a href="javascript:void(0);" class="btn btn-mini"
                                                       ...
                                                       onclick="remover(<?php echo $curso["idregra_curso"]; ?>)"><i
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            $("#cursos").fcbkcomplete({
                json_url: "/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/<?= $url[3]; ?>/json/associar_cursos",
                addontab: true,
                height: 10,
                maxshownitems: 10,
                cache: true,
