                        ...
                        <div class="tab-pane active" id="tab_editar">
                            ...
                            <form method="post" onsubmit="return validateFields(this, regras)"
                                ...
                                <? if ($url[4] == "editar") {
                                    ...
                                    if(!$_POST['codigo']) $_POST['codigo'] = geraCupons();
                ...
                foreach($fieldset["campos"] as $campoid => $campo) {
                    ...
                    if ($campo["botao_hide"]){                  
                        if ($campo['tipo'] == 'select') { ?>
                            ...
                            if (aux_d == 'V'){
                                $('#<?= $campo["id"]; ?> option[value="V"]').attr('selected','selected');
                                $('#div_form_<?= $campo["iddiv2"]; ?>').show();
                                for (var i = 0; i < regras.length; i++){
                                    if(regras[i] == 'required,form_<?= $campo["iddiv"]; ?>,<?= $idioma["porcentagem_vazio"] ?>')
                                        regras.splice(i, 1); 
                                }
                            } else {
                                $('#<?= $campo["id"]; ?> option[value="P"]').attr('selected','selected');
                                $('#div_form_<?= $campo["iddiv"]; ?>').show();
                                for (var i = 0; i < regras.length; i++){ 
                                    if(regras[i] == 'required,form_<?= $campo["iddiv2"]; ?>,<?= $idioma["valor_vazio"] ?>') {
                                        regras.splice(i, 1); 
                                    }
                                }
                            }
                            $('#<?= $campo["id"]; ?>').change(function() {
                                var contem = false;
                                var remover = 0;
                                aux_d = $('#form_tipo_desconto').attr('value');
                                if (aux_d == 'P'){
                                    ...
                                    for (var i = 0; i < regras.length; i++){ 
                                        if(regras[i] == 'required,form_<?= $campo["iddiv2"]; ?>,<?= $idioma["valor_vazio"] ?>') 
                                            remover = i;
                                        else if (regras[i] == 'required,form_<?= $campo["iddiv"]; ?>,<?= $idioma["porcentagem_vazio"] ?>') 
                                            contem = true;
                                    }
                                    if (remover != 0)
                                        regras.splice(remover, 1);                                
                                    if (!contem)
                                        regras.push("required,form_<?= $campo["iddiv"]; ?>,<?= $idioma["porcentagem_vazio"] ?>");                            
                                } else if(aux_d == 'V') {
                                    ...
                                    for (var i = 0; i < regras.length; i++){
                                        if(regras[i] == 'required,form_<?= $campo["iddiv"]; ?>,<?= $idioma["porcentagem_vazio"] ?>')
                                            remover = i;
                                        else if (regras[i] == 'required,form_<?= $campo["iddiv2"]; ?>,<?= $idioma["valor_vazio"] ?>')
                                            contem = true;
                                    }
                                    if (remover != 0)
                                        regras.splice(remover, 1);
                                    if (!contem)
                                        regras.push("required,form_<?= $campo["iddiv2"]; ?>,<?= $idioma["valor_vazio"] ?>"); 
                                }
                            });
                        <?
                        }
