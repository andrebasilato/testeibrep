...
<div class="container-fluid">
    ...
    <script type="text/javascript">
            ...
            foreach($fieldset["campos"] as $campoid => $campo) {
                if(is_array($campo["validacao"])){
                    foreach($campo["validacao"] as $tipo => $mensagem) {
                        ...
                        <? } elseif($campo["id"] != 'form_bandeiras' && $campo["id"] != 'form_formas_pagamento') { ?>
        ...
        jQuery(function ($) {
            ...
            function mudaHomologado() {
                homologado = $("#form_homologado").attr("value");

                //var contemDataLicitacao = false;
                var contemNumeroEstabelecimento = false;
                var contemToken = false;

                //Se homologado for SIM
                if (homologado == 'S') {
                    $("#div_form_numero_estabelecimento").fadeIn(200);
                    $("#div_form_token").fadeIn(200);

                    for (var i = 0; i < regras.length; i++) {
                        if (regras[i] == "required,form_numero_estabelecimento,<?= $idioma["numero_estabelecimento_vazio"] ?>") {
                            contemNumeroEstabelecimento = true;
                        }

                        if (regras[i] == "required,form_token,<?= $idioma["token_vazio"] ?>") {
                            contemToken = true;
                        }
                    }

                    if (!contemNumeroEstabelecimento) {
                        regras.push("required,form_numero_estabelecimento,<?= $idioma["numero_estabelecimento_vazio"] ?>");
                    }

                    if (!contemToken) {
                        regras.push("required,form_token,<?= $idioma["token_vazio"] ?>");
                    }
                } else {
                    $("#div_form_numero_estabelecimento").fadeOut(100);
                    $("#div_form_token").fadeOut(100);
            ...
            }//Fim função mudaHomologado()
            mudaHomologado();

            $('#form_homologado').change(function() {
                mudaHomologado();
            });
