...
if(count($dadosArray) > 0){
  ...
  foreach($dadosArray as $ind => $dados){	
	...
	$sheet->setCellValue('G'.$linha, $dados['estado']);
	$sheet->setCellValue('H'.$linha, $dados['cidade']);
	$sheet->setCellValue('I'.$linha, $dados['endereco']." ".$dados['bairro']."; Complemento: ".$dados['complemento']."; CEP: ".$dados['cep'] );
	$sheet->setCellValue('J'.$linha, $estadocivil[$config["idioma_padrao"]][$dados["estado_civil"]]);
	$sheet->setCellValue('K'.$linha, formataData($dados["data_nasc"],'br',0));
	$sheet->setCellValue('L'.$linha, formataData($dados["data_cad"],'br',1));
	$sheet->setCellValue('M'.$linha, html_entity_decode(strip_tags(str_replace("<br />", "\n", $dados['sindicatos']))));
  }
}
