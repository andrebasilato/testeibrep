...
$sheet->setCellValue('A5','Nome do aluno(a): ' . $dadosArray[0]['nome'] . '. Gerado dia '.date('d/m/Y H:i:s').' por '.$usuario['nome'].' ('.$usuario['email'].')');
$linhaBase = 4;
if (count($dadosArray) > 0) {
    $sheet->insertNewRowBefore($linhaBase, count($dadosArray));
    foreach ($dadosArray as $ind => $dados) {
        $linha = $linhaBase + $ind;
        $sheet->setCellValue('A'.$linha, $dados['idhistorico']);
        $sheet->setCellValue('B'.$linha, $dados['idava']);
        $sheet->setCellValue('C'.$linha, $dados['ava']);
        $sheet->setCellValue('D'.$linha, $dados['acao']);
        $sheet->setCellValue('E'.$linha, $dados['oque']);
        $sheet->setCellValue('F'.$linha, $dados['id']);
        if ($dados["total_perguntas"]) {
            $sheet->setCellValue('G'.$linha, $dados['total_perguntas_corretas']."/".$dados['total_perguntas']);
        } else {
            $sheet->setCellValue('G'.$linha, "--");
        }
        $sheet->setCellValue('H'.$linha, formataData($dados['data_cad'], 'br', 1));
    }
}
