...
<div class="container">
    ...
    <div class="row">
      ...
      <p>Para gerar esse relatório vá até a listagem de fechamento de caixa e clique no botão "VISUALIZAR" do fechamento de caixa que deseja visualizar.</p>
						...
						if ($campo['tipo'] == 'select') { ?>
							...
							$('#<?= $campo["id"]; ?>').change(function() {
								...
								if (aux_d == 'PER'){
									...
									if (div1_obr)
										regras.push("required,form_<?= $campo["iddiv"]; ?>,<?= $idioma["de_vazio"] ?>"); 
									if (div2_obr)
										regras.push("required,form_<?= $campo["iddiv2"]; ?>,<?= $idioma["ate_vazio"] ?>"); 
									...
									for (var i = 0; i < regras.length; i++){ 
										if(regras[i] == 'required,form_<?= $campo["iddiv"]; ?>,<?= $idioma["de_vazio"] ?>')
										   remover_1 = i;
									}
									if (remover_1 != 0)
										regras.splice(remover_1, 1);
									for (var i = 0; i < regras.length; i++){ 
										if (regras[i] == 'required,form_<?= $campo["iddiv2"]; ?>,<?= $idioma["ate_vazio"] ?>')
											remover_2 = i;
									}
									if (remover_2 != 0)
