...
if(count($dadosArray) > 0){
  ...
  foreach($dadosArray as $ind => $dados){	
	$linha = $linhaBase + $ind;	
	$sheet->setCellValue('A'.$linha, $dados["idlog"]);
	$sheet->setCellValue('B'.$linha, $dados['idmatricula']);
	$sheet->setCellValue('C'.$linha, formataData($dados["data_cad"],'br',1));
	$sheet->setCellValue('D'.$linha, $dados['cod_transacao']);
	$sheet->setCellValue('E'.$linha, $dados['retorno']);
	$sheet->setCellValue('F'.$linha, $dados['string_envio']);
  }
}
