...
if(count($dadosArray) > 0){
    ...
    foreach($dadosArray as $ind => $dados){
        ...
        $sheet->setCellValue('B'.$linha, ' '.$dados["documento"]);
      	$sheet->setCellValue('C'.$linha, $dados["email"]);
      	$sheet->setCellValue('D'.$linha, ' '.$dados["documento"]);
        $sheet->setCellValue('E'.$linha, 'IBREP');
        $sheet->setCellValue('F'.$linha, 'ALUNO');
        $sheet->setCellValue('G'.$linha, str_replace("-","",$dados["validade"]));
        $sheet->setCellValue('H'.$linha, '1');
    }
}
