...
if(count($dadosArray) > 0){
  ...
  foreach($dadosArray as $ind => $dados){	
	$linha = $linhaBase + $ind;	
	$sheet->setCellValue('A'.$linha, $dados["idfornecedor"]);
	$sheet->setCellValue('B'.$linha, $dados['nome']);	
	$sheet->setCellValue('C'.$linha, $dados['documento']);
	$sheet->setCellValue('D'.$linha, $dados['fax']);
	$sheet->setCellValue('E'.$linha, $dados['telefone']);
	$sheet->setCellValue('F'.$linha, $dados['email']);
	$sheet->setCellValue('G'.$linha, $dados['sindicato']);
  }
}
