...
class Relatorio extends Core {
	function gerarRelatorio(){
		...
		$this->sql = "SELECT
						  ".$this->campos."
						FROM fornecedores f
							LEFT OUTER join estados es ON (f.idestado = es.idestado)
							LEFT OUTER join cidades ci ON (f.idcidade = ci.idcidade)
							INNER JOIN sindicatos i ON (i.idsindicato=f.idsindicato)
							WHERE f.ativo='S'
						  ";
						  
	if(!$_GET["q"]["1|f.idsindicato"] && $_SESSION['adm_gestor_sindicato'] != 'S'){
		$this->sql .= ' and f.idsindicato in ('.$_SESSION['adm_sindicatos'].')';	
	}						  			  
