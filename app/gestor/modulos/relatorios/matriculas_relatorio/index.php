...
while($situacao = mysql_fetch_assoc($seleciona)) {
   $config['situacoesArray'][$situacao['idsituacao']] = $situacao;
    ...
    if ($url[3] == 'html' || $url[3] == 'xls') {
        $sql = " SELECT idsituacao FROM matriculas_workflow WHERE ativo = 'S' and ativa = 'S' ";
        $wf_emcurso = $relatorioObj->retornarLinha($sql);
        ...
        $relatorioObj->set('pagina', 1)
            ...
            ->set(
                'campos',
                'ma.idmatricula, ma.data_registro, ma.data_matricula, ma.idturma, ma.data_conclusao,
                ma.ativo, ma.idsindicato, ma.idsituacao, ma.porcentagem_manual, ma.porcentagem, 
                ma.idpessoa, ma.data_cad, ma.valor_contrato,
                pe.nome as cliente, pe.email as emailCli, pe.documento, pe.ultimo_acesso,
                pe.telefone, pe.celular, po.razao_social AS cfc,
                ve.nome as vendedor, cu.nome as curso, o.nome as oferta,
                mw.nome as situacao_wf_nome,
                CASE WHEN ma.faturada = "S" THEN "Sim" WHEN ma.faturada = "CUPOM" THEN "Cupom" else "Não" END AS matricula_faturada,
                mw.sigla as situacao_wf_sigla,
				tu.nome as turma,
                est.nome as estado,
                cid.nome as cidade,
                (SELECT data_cad FROM matriculas_historicos WHERE para = ' . $wf_emcurso['idsituacao'] . ' AND idmatricula = ma.idmatricula ORDER BY data_cad DESC LIMIT 1) as em_curso,
                cfcs_valores_cursos.valor_por_matricula as valor_cfc,
                sindicatos_valores_cursos.valor_por_matricula as valor_sindicato,
