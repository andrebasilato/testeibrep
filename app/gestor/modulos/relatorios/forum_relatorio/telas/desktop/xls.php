...
if($_GET['tipo'] == 'S'){
$template = $_SERVER['DOCUMENT_ROOT']."/".$url[0]."/modulos/".$url[1]."/".$url[2]."/telas/".$config["tela_padrao"]."/"."xls_pt_br_detalhado.xls";
}else{
...
if(count($dadosArray) > 0){
  ...
  foreach($dadosArray as $ind => $dados){
	...
	$sheet->setCellValue('A'.$linha, $dados["idtopico"]);
	$sheet->setCellValue('B'.$linha, formataData($dados["periode_de"],"br",0));	
	$sheet->setCellValue('C'.$linha, $dados["nome"]);
	$sheet->setCellValue('D'.$linha, $dados["topico"]);
	if($_GET['tipo'] == 'S'){		
		$sheet->setCellValue('E'.$linha, $dados["postagens_alunos_xls"]);		
	}
  }
  $linha++;  
}

//print_r2($dadosArray,true);
