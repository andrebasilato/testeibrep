...
if(count($dadosArray) > 0){
  ...
  foreach($dadosArray as $ind => $dados){
    ...
    $sheet->setCellValue('K'.$linha, $idpedido);
    ...
    $sheet->setCellValue('L'.$linha, $cupom);
    
    $sheet->setCellValue('M'.$linha, number_format(max($dados["porcentagem"], $dados["porcentagem_manual"]), 2, ",", "."));
	$sheet->setCellValue('N'.$linha, $dados["vendedor"]);
	$sheet->setCellValue('O'.$linha, $dados["cupom_nota_fiscal"]);
	$sheet->setCellValue('P'.$linha, floatval(number_format($dados["valor_contrato"], 2, ".", "")));
  }
  $linha++;
  $sheet->setCellValue('P'.$linha, '=SUM(P'.$linhaBase.':P'.($linha-1).')');//TotAL
}
