   ...
   $config['situacoesArray'][$situacao['idsituacao']] = $situacao;
    ...
    if ($url[3] == 'html' || $url[3] == 'xls') {
        $relatorioObj->set('pagina', 1)
            ...
            ->set(
                'campos', 'ma.*, pe.nome as cliente, folhas_matriculas.numero_registro, folhas_matriculas.numero_ordem,
                 folhas.numero_livro, folhas.numero_folha, pe.cnh, pe.email as emailCli, pe.documento,
                  ve.nome as vendedor, cu.nome as curso, o.nome as oferta, mw.nome as situacao_wf_nome,
                   mw.sigla as situacao_wf_sigla, tu.nome as turma, folhas.idfolha'
        );
