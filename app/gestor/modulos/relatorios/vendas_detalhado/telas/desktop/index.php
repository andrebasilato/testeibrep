              ...
              <div class="control-group">
                <div class="controls">
                  <label class="checkbox">
                    <input name="marcar_todas" id="marcar_todas" onclick="selecionaTodos()" value="" type="checkbox">
                    <em><?php echo $idioma['marcar_todas']; ?></em>
                  </label>
                </div>
                <label class="control-label" for="form_idsituacao">Colunas de exibição:</label>
                <div class="controls" id="div_colunas">
                  <?php foreach($colunas as $ind => $coluna) { ?>
                    <label class="checkbox">
                      <input id="colunas" name="colunas[]" value="<?php echo $ind; ?>" type="checkbox" <?php if(in_array($ind,$colunasPadrao)) {?>checked="checked"<?php } ?> /><?php echo $idioma[$coluna]; ?>
                    </label>
                  <?php } ?>
                </div>
              </div>
	...
	<script type="text/javascript">
            ...
            <? 
					...
					if ($campo["botao_hide"]){                    
						if ($campo["tipo"] == "select") {
						  if ($campo['id'] == 'form_forma_pagamento') {
						?>
						...
						<?
						  } else if ($campo['id'] == 'form_tipo_data_filtro') {  ?>
		...
		function liberaCamposFormasPagamento(valorCampo){
			...
			if (valorCampo == 2 || valorCampo == 3) {
				...
				if(indBandeira > -1) { regras.splice((indBandeira), 1); qtd_removidos++;}
				if(indAutoriza > -1) { regras.splice((indAutoriza-qtd_removidos), 1); qtd_removidos++;}*/
		...
		$('#idescola').val([<?= $empreendimentosSelect ?>]);
		$('.select2').select2();
	</script>
