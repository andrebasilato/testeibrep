<?php 
	if($_GET['update'] == 'up'){
		$sql = "UPDATE atendimentos SET idempreendimento='".$_GET["q"]["1|e.idempreendimento"]."' WHERE idassunto = '".$_GET["q"]["1|ate.idassunto"]."' AND idsubassunto = '".$_GET["q"]["1|ate.idsubassunto"]."'";
...
<div class="container"> 
    ...
    <div class="row">
      ...
      <p>
      <form method="get" action="/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/script" id="formRelatorio" class="form-horizontal" target="_blank">  
      <input name="update" value="up" type="hidden" />  
	  	<? $relatorioObj->GerarFormulario("formscript",$linha,$idioma); ?>
        <div class="form-actions">
          <input type="submit" class="btn btn-primary" value="enviar" onClick="document.action = '/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/script'">
          &nbsp;         
...
<?php incluirLib("rodape",$config,$usuario); ?>
	...
	<script type="text/javascript">
		...
		jQuery(document).ready(function($) { 
			<?
			foreach($config["formscript"] as $fieldsetid => $fieldset) {
				foreach($fieldset["campos"] as $campoid => $campo) {
					if($campo["mascara"]){ ?>
