...
if(count($dadosArray) > 0){
    ...
    foreach($dadosArray as $ind => $dados){
        ...
        $sheet->setCellValue('B'.$linha, $dados["idmatricula"]);
      	$sheet->setCellValue('C'.$linha, $dados["aluno"]);
      	$sheet->setCellValue('D'.$linha, $dados["tipo"]);
        $sheet->setCellValue('E'.$linha, formataData($dados["data_cad"],"br",1));
        if ($dados["aluno_acadastrou"]) {
          $sheet->setCellValue('F'.$linha, "Aluno: ".$dados["aluno_acadastrou"]);
        } else {
          $sheet->setCellValue('F'.$linha, "Usuário: ".$dados["usuario_cadastrou"]);
        }
    }
}
