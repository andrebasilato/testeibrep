...
if(count($dadosArray) > 0){
  ...
  foreach($dadosArray as $ind => $dados){	
	...
	$sheet->setCellValue('D'.$linha, $dados['profissao']);
	$sheet->setCellValue('E'.$linha, $dados['email']);
	$sheet->setCellValue('F'.$linha, $dados['telefone']);
    $sheet->setCellValue('G'.$linha, $dados['celular']);
	$sheet->setCellValue('H'.$linha, $dados['estado']);
	$sheet->setCellValue('I'.$linha, $dados['cidade']);
	$sheet->setCellValue('J'.$linha, $dados['endereco']." ".$dados['bairro']."; Complemento: ".$dados['complemento']."; CEP: ".$dados['cep'] );
	$sheet->setCellValue('K'.$linha, $estadocivil[$config["idioma_padrao"]][$dados["estado_civil"]]);
	$sheet->setCellValue('L'.$linha, formataData($dados["data_nasc"],'br',0));
	$sheet->setCellValue('M'.$linha, formataData($dados["data_cad"],'br',1));
	$sheet->setCellValue('N'.$linha, html_entity_decode(strip_tags(str_replace("<br />", "\n", $dados['sindicatos']))));
  }
}
