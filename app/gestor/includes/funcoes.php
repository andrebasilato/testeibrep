...
function incluirTela($tela,$config,$informacoes = null){
	$url = $GLOBALS["url"];
	$root = $_SERVER['DOCUMENT_ROOT']."/gestor/modulos/".$url[1]."/".$url[2];
...
function listarFuncionalidades($url) {
	...
	if ($pastas != "") {
		foreach ($pastas as $listar) {
			...
			include_once $diretorio . '/' . $listar . '/idiomas/' . $GLOBALS['config']['idioma_padrao'] . '/config.php';
			$nome = $idioma['funcionalidade'];
			...
			if (verificaModulos($url, $listar, false)) {
