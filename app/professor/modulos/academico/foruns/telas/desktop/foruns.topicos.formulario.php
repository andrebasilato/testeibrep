        ...
        <div class="row-fluid">
          <div class="span12">
            <div class="forum-form marge30-up">
              <div class="forum-base-titulo corbgpadrao"><?php if($url[6] != "moderar") { echo $idioma["criar_topico"]; } else { echo $idioma["moderar_topico"]; } ?></div>
              <div>
                <?php
				if($url[6] == "moderar") {
					$action = '/'.$url[0].'/'.$url[1].'/'.$url[2].'/'.$url[3].'/'.$url[4].'/'.$url[5].'/mensagens';
				} else {
					$action = '/'.$url[0].'/'.$url[1].'/'.$url[2].'/'.$url[3].'/'.$url[4];
				}
				?>
                <form method="post" action="<?php echo $action; ?>" onsubmit="return valida(this, regras);" enctype="multipart/form-data" target="_parent">
                  <input name="acao" id="acao" type="hidden" value="<?php if($url[6] != "moderar") { ?>salvar_topico<?php } else { ?>moderar_topico<?php } ?>" />
                  <input name="idforum" id="idforum" type="hidden" value="<?php echo $url[3]; ?>" />
                  <?php if($url[6] != "moderar") { ?>
                  ...
                  <br clear="all"/>
                  <textarea id="form_mensagem" name="<?php if($url[6] != "moderar") { ?>mensagem<?php } else { ?>moderar<?php } ?>"><?php if($topico["moderado"] == "S") { echo nl2br($topico["moderado_mensagem"]); } else { echo nl2br($topico["mensagem"]); } ?></textarea>
                  <?php if($url[6] != "moderar" && $linhaObj->verificaPermissao($forum["permissoes"], $url[0]."|topicos|8", false)) { ?>
...
<script type="text/javascript">
  ...
  regras.push("required,form_mensagem,<?php echo $idioma["mensagem_vazio"]; ?>");
	...
	jQuery(document).ready(function($) {
		var editor = CKEDITOR.replace( '<?php if($url[6] != "moderar") { ?>mensagem<?php } else { ?>moderar<?php } ?>', {
			height: 290,
			width: '100%',
			toolbar: [
