              ...
              <div class='section-body'>
                <div class='row-fluid'>
					<table class="table table-striped ">
						<thead>
							<tr>
							  <th><?= $idioma["listagem_oferta"]; ?></th>
							  <th><?= $idioma["listagem_situacao"]; ?></th>
							  <th><?= $idioma["listagem_inicio_matricula"]; ?></th>
							  <th><?= $idioma["listagem_fim_matricula"]; ?></th>
							</tr>
						</thead>
						<tbody>
							<?php if(count($ofertas) > 0) { ?>
								<?php foreach($ofertas as $ind => $oferta) { ?>
									<tr>
										<td><?php echo $oferta["nome"]; ?></td>
										<td><span class="label" style="color:#<?php echo $oferta["cor_nome"]; ?>; background-color:#<?php echo $oferta["cor_bg"]; ?>"><?php echo $oferta["situacao"]; ?></span></td>
										<td><?php echo formataData($oferta["data_inicio_matricula"],'pt',0); ?></td>
										<td><?php echo formataData($oferta["data_fim_matricula"],'pt',0); ?></td>										
									</tr>
								<?php } ?>
							<?php } else { ?>
								<tr>
              ...
              <div class='section-body'>
                <div class='row-fluid'>
					<table class="table table-striped ">
						<thead>
							<tr>
							  <th><?= $idioma["listagem_ava"]; ?></th>
							  <th><?= $idioma["listagem_data_cad"]; ?></th>
							</tr>
						</thead>
						<tbody>
							<?php if(count($avas) > 0) { ?>
								<?php foreach($avas as $ind => $ava) { ?>
									<tr>
										<td><?php echo $ava["nome"]; ?></td>
										<td><?php echo formataData($ava["data_cad"],'pt',1); ?></td>									
									</tr>
								<?php } ?>
							<?php } else { ?>
								<tr>
