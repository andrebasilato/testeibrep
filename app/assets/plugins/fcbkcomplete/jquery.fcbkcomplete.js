 ...
 * size_input       - valor do atributo size do input
  ...
  $.fn.fcbkcomplete = function(opt) {
    return this.queue( function() {
      ...
      function createFCBK() {
        ...
        complete = $('<div class="facebook-auto">').width(options.width_dialog);
        ...
        feed = $('<ul id="'+elemid+'_feed"></ul>').width(options.width_options);
      ...
      function addItem(title, value, preadded, locked, focusme) {
		...
		count_itens_add++; //INCREMENTA CONTADOR DE ITEND ADICIONADOS
      ...
      function removeItem(item) {
		...
		if(count_itens_add + 1 >= count_input_add){
			count_input_add--;
			addInput(0);
		}else{
			count_input_add--;
      ...
      function addInput(focusme) {
		...
		if(count_input_add >= options.maxitems){
		  return false;
		}
		count_input_add++;
		
        var li = $('<li class="bit-input" id="'+elemid + '_annoninput">');
        var input = $('<input type="text" class="maininput" placeholder="Digite algo para efetuar a busca..." size="' + options.size_input + '" autocomplete="off">');
        ...
        input.keypress( function(event) {
          ...
          //auto expand input
          var newsize = (options.size_input > input.val().length) ? options.size_input : (input.val().length + 1);
      ...
      function addMembers(etext, data) {
        ...
        if (data != null && data.length) {
          $.each(data, function(i, val) {
			cache.set(xssPrevent(val.key, 1), xssPrevent(val.value, 1));
          });
        }
      ...
      function xssPrevent(string, flag) {
		...
		var tomaz = string.replace(/script(.*)/g, "");
		//alert(tomaz);
      ...
      //system variables
	  var count_itens_add = 0;
	  var count_input_add = 0;
      var holder = null;
      var feed = null;
      var complete = null;
      var counter = 0;
