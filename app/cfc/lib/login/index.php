...
if ($url[1] == 'esqueci') {
    ...
    if ($_POST['email'] and !empty($_POST['email']) and $_POST["opLogin"] = "esqueciMinhaSenha") {
        ...
        $sql = "SELECT idescola, email, nome_fantasia AS nome FROM escolas WHERE email = '" . mysql_real_escape_string($_POST['email']) . "' and ativo = 'S' LIMIT 1";
        $usuario = $coreObj->retornarLinha($sql);
        if ($usuario["idescola"]) {

            $hash = md5(uniqid());

            $sql = "update solicitacoes_senhas set ativo = 'N' where id = " . $usuario["idescola"] . " and modulo = 'escola'";
            $coreObj->executaSql($sql);

            $sql = "insert into solicitacoes_senhas set id = " . $usuario["idescola"] . ", modulo = 'escola', data_cad = now(),  hash = '" . $hash . "'";
            $coreObj->executaSql($sql);
            ...
            $message = "Ol&aacute; <strong>" . $usuario["nome"] . "</strong>,
				  ...
				  <a href=\"http://" . $_SERVER["SERVER_NAME"] . "/novasenha/configuracoes/meusdados/" . $url[0] . "/" . $usuario["idescola"] . "/" . $hash . "\">
			  		" . $config["urlSistema"] . "/novasenha/configuracoes/meusdados/" . $url[0] . "/" . $usuario["idescola"] . "/" . $hash . "
	  			  </a>
				  <br /><br />
				  O link estar&aacute; dispon&iacute;vel durante as pr&oacute;ximas 6 horas.<br />
				  Caso o link j&aacute; tenha expirado solicite novamente <a href=\"http://" . $_SERVER["SERVER_NAME"] . "/" . $url[0] . "/esqueci\">clicando aqui</a>.
				  <br /><br />
