...
<div class="container-fluid" >
  ...
  <form action="/<?= $url[0]; ?>/<?= $url[1]; ?>/<?= $url[2]; ?>/<?= $url[3]; ?>/<?= $url[4]; ?>/<?= $url[5]; ?>" method="post" onSubmit="return valida(this, regras);" target="_parent">	  
	<?php echo $idioma["iddeclaracao"]; ?>
    <br />
    <select name="iddeclaracao" id="iddeclaracao" style="width:auto">
      <option value=""></option>
	  <? foreach($declaracoes as $declaracao) { ?>
        <option value="<?= $declaracao["tipo"]; ?>|<?= $declaracao["iddeclaracao"]; ?>" <?php if ($declaracao["iddeclaracao"] == $url[7]) { ?> selected="selected" <?php } ?>><?= $declaracao["nome"]; ?></option>
	  <? } ?>
    </select>
    <br /><br />
...
<script type="text/javascript">
    ...
    $("#data_prevista_conclusao").datepicker($.datepicker.regional["pt-BR"]);
        ...
        regras.push("required,iddeclaracao,<?php echo $idioma["iddeclaracao_vazio"]; ?>");
