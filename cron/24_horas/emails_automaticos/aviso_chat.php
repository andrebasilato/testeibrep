...
if ($email['dia_semanal']) {
	...
	$data_chat_semana_final = new DateTime();
	$data_chat_semana_final->modify('+7 days');
	...
	$sql = 'select
			...
			from
				avas_chats
			where					
				DATE_FORMAT(inicio_entrada_aluno,"%Y-%m-%d") > "' . $data_atual->format('Y-m-d') . '" and
				DATE_FORMAT(inicio_entrada_aluno,"%Y-%m-%d") < "' . $data_chat_semana_final->format('Y-m-d') . '"	';			
} else {
	...
	$sql = 'select
			...
			from
				avas_chats
			where					
				DATE_FORMAT(inicio_entrada_aluno,"%Y-%m-%d") = "' . $data_chat_dia->format('Y-m-d') . '" ';
}
