...
$data_inadimplencia = new DateTime();
$data_inadimplencia->modify('-10 days');
	...
	where 
			...
			(
				(
					...
					where 
						...
						c.data_vencimento <= "' . $data_inadimplencia->format('Y-m-d') . '" and
						c.ativo_painel = "S"
				) > 0
			)
			or
		...
		(
			...
			where 
				...
				eal.data_cad >= "' . $data_inadimplencia->format('Y-m-d') . '" and 
				eal.ativo = "S" and 
				eal.tipo = "inadi"
		) = 0	';
